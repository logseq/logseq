...
(defn- capture-stderr!
  [f]
  (test-helper/with-stderr-write-capture
    (fn [buffer]
      (p/let [result (f)]
        {:buffer buffer
         :result result}))))
  ...
  (async done
         (let [data-dir (node-helper/create-tmp-dir "db-worker")]
           (-> (p/let [cfg-path (node-path/join (node-helper/create-tmp-dir "cli") "cli.edn")
                       ...
                       remove-block-result (run-cli ["--graph" "content-graph" "remove" "block" "--id" (str block-id)] data-dir cfg-path)
                       remove-block-payload (parse-json-output remove-block-result)
                       show-deleted-block-result (run-cli ["--graph" "content-graph" "show" "--id" (str block-id)] data-dir cfg-path)
                       show-deleted-block-payload (parse-json-output show-deleted-block-result)
                       ...
                       show-deleted-page-result (run-cli ["--graph" "content-graph" "show" "--id" (str page-id)] data-dir cfg-path)
                       show-deleted-page-payload (parse-json-output show-deleted-page-result)
                 ...
                 (is (= "ok" (:status remove-block-payload)))
                 (is (= "error" (:status show-deleted-block-payload)))
                 (is (contains? #{:entity-not-found :exception}
                                (keyword (get-in show-deleted-block-payload [:error :code]))))
                 (is (string/includes? (string/lower-case (or (get-in show-deleted-block-payload [:error :message]) ""))
                                       "entity not found"))
                 (is (= "ok" (:status remove-page-payload)))
                 (is (= "error" (:status show-deleted-page-payload)))
                 (is (contains? #{:entity-not-found :exception}
                                (keyword (get-in show-deleted-page-payload [:error :code]))))
                 (is (string/includes? (string/lower-case (or (get-in show-deleted-page-payload [:error :message]) ""))
                                       "entity not found"))
                 (is (= "ok" (:status stop-payload)))
                 (done))
               (p/catch (fn [e]
  ...
  (async done
         (let [data-dir (node-helper/create-tmp-dir "db-worker-verbose")
           ...
           (-> (p/let [cfg-path (node-path/join (node-helper/create-tmp-dir "cli") "cli.edn")
                       _ (fs/writeFileSync cfg-path "{:output-format :json}")
                       _ (run-cli ["graph" "create" "--graph" repo] data-dir cfg-path)
                       {:keys [buffer result]} (capture-stderr!
                                                (fn []
                                                  (run-cli ["--verbose" "--graph" repo "graph" "info"] data-dir cfg-path)))
         ...
         (let [data-dir (node-helper/create-tmp-dir "db-worker-list-tag-default-sort")
           ...
           (-> (p/let [cfg-path (node-path/join (node-helper/create-tmp-dir "cli") "cli.edn")
                       ...
                       update-result (run-cli ["--graph" repo "upsert" "tag" "--id" (str tag-a-id) "--name" "SortTagA-Renamed"]
                                              data-dir cfg-path)
                       _ (p/delay 80)
                       default-result (run-cli ["--graph" repo "list" "tag" "--no-include-built-in" "--fields" "title,id,updated-at"] data-dir cfg-path)
                       default-payload (parse-json-output default-result)
                       explicit-result (run-cli ["--graph" repo "list" "tag" "--no-include-built-in" "--sort" "updated-at" "--fields" "title,id,updated-at"] data-dir cfg-path)
                       explicit-payload (parse-json-output explicit-result)
                       default-ids (ordered-item-ids-by-title-set default-payload titles)
                       explicit-ids (ordered-item-ids-by-title-set explicit-payload titles)
                       stop-payload (stop-repo! data-dir cfg-path repo)]
  ...
  (async done
         (let [data-dir (node-helper/create-tmp-dir "db-worker-list-property-default-sort")
           ...
           (-> (p/let [cfg-path (node-path/join (node-helper/create-tmp-dir "cli") "cli.edn")
                       ...
                       _ (p/delay 80)
                       default-result (run-cli ["--graph" repo "list" "property" "--no-include-built-in" "--fields" "title,id,updated-at"] data-dir cfg-path)
                       default-payload (parse-json-output default-result)
                       explicit-result (run-cli ["--graph" repo "list" "property" "--no-include-built-in" "--sort" "updated-at" "--fields" "title,id,updated-at"] data-dir cfg-path)
                       explicit-payload (parse-json-output explicit-result)
                       default-ids (ordered-item-ids-by-title-set default-payload titles)
                       explicit-ids (ordered-item-ids-by-title-set explicit-payload titles)
                       stop-payload (stop-repo! data-dir cfg-path repo)]
