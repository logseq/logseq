(ns frontend.colors
  ...
  (:require [clojure.string :as string]
            ...
            [frontend.colors.named-loader :as named-loader]
...
(defn parse-css-color->hex
  "Parse a computed CSS color value (hex / hsl(...) / rgb(...)) into a hex
   string. Returns nil for blank or unparsable input; NaN channels are
   rejected and RGB channels clamp to [0..255]."
  [s]
      ...
      (let [parts (-> v
        ...
        (when (>= (count parts) 3)
          (let [[h s l] (map js/parseFloat (take 3 parts))]
            (when (every? #(and (number? %) (not (js/isNaN %))) [h s l])
      ...
      (let [parts (-> v
        ...
        (when (>= (count parts) 3)
          (let [[r g b] (map #(js/parseInt %) (take 3 parts))
                byte-hex (fn [n] (-> n (max 0) (min 255) (.toString 16) (.padStart 2 "0")))]
            (when (every? #(and (number? %) (not (js/isNaN %))) [r g b])
...
;; through the `icon` color wrapper and the picker's color controls.
...
(defn- bounded-memoize
  "Wrap f with a memo cache capped at `max-entries`. When the cap is
   ...
   for our caller (adjust-for-contrast) stays well under
...
(def ^{:doc "Adjust `picked-hex` toward sufficient WCAG contrast against `surface-hex`.
  ...
  Memoized via a 256-entry bounded cache (evicts an arbitrary half on
  overflow), keyed on [picked surface target]."}
  adjust-for-contrast
...
;; --- Lazy XKCD dictionary ---------------------------------------------------
;; The ~950-entry XKCD set lives in the :colors-named lazy module so it stays
;; off the startup bundle. `load-named-dict!` fires on picker open; until the
;; module lands every consumer below degrades gracefully (hex, var(--…) and
;; the eager 148 CSS names keep resolving; XKCD names resolve once loaded).
;; Module access goes through frontend.colors.named-loader — lazy in browser
;; builds, swapped for the eager variant via :ns-aliases where there are no
;; :modules (node tests).
(def *named-ready?
  "Ready flag for the XKCD module; UI can subscribe to repaint on load."
  named-loader/*ready?)

(defn load-named-dict!
  "Kick off loading the XKCD color-name module. Idempotent."
  []
  (named-loader/load!))

(defn- named-colors*
  "The XKCD name->hex map, or nil while the lazy module hasn't loaded."
  []
  (named-loader/named-colors))

(defn- named-hex->name*
  [stripped]
  (named-loader/named-hex->name stripped))
...
(defn dict-exact->hex
  ...
  (when (string? s)
    (let [norm (normalize-name s)]
      (when (and norm (not (contains? color-blacklist norm)))
        (->hex-with-hash (get (named-colors*) norm))))))

(def ^:private *sorted-keys-cache
  "Sorted vector of normalized XKCD keys. Filled once the lazy module has
   landed — deliberately NOT a delay: forcing a delay before the module
   loads would cache the empty state forever."
  (atom nil))

(defn- sorted-keys
  []
  (or @*sorted-keys-cache
      (when-let [nc (named-colors*)]
        (reset! *sorted-keys-cache (vec (sort (keys nc)))))))
  ...
  (when (and (string? s) (>= (count s) 2))
    (let [norm (normalize-name s)]
      (when (and norm
        ...
        (when-let [match (->> (sorted-keys)
                              (some #(when (string/starts-with? % norm) %)))]
          {:hex (->hex-with-hash (get (named-colors*) match))
           :name match})))))
