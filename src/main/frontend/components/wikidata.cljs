...
(defn- extract-time-value
  "Extract time value from a time-type claim, return as epoch ms (LOCAL midnight).
   Returns nil when JS parses the time to an Invalid Date (year-only like
   +1452-00-00, or BCE) so a NaN epoch never flows through to a property.
   Wikidata dates are day-precision at UTC midnight; we rebuild them at local
...
(defn- filter-claims-by-rank
  "Wikidata claims carry a rank (preferred/normal/deprecated). Drop deprecated;
   if any preferred exist keep only those, else keep all normal. Prevents a
   deprecated/historical value from being the one that lands."
  [claims]
  (let [ranked (remove #(= "deprecated" (get % "rank")) claims)
        preferred (filter #(= "preferred" (get % "rank")) ranked)]
    (if (seq preferred) preferred ranked)))
  ...
  (let [claims (get entity-data "claims" {})]
    (->> wikidata-property->logseq
         (map (fn [[prop-id logseq-info]]
                (let [prop-claims (filter-claims-by-rank (get claims prop-id []))
                      values (->> prop-claims
            ...
            (let [;; Get appropriate image property
              ...
              (when image-prop
                (p/let [;; Check cancellation again
                  ...
                  ;; Report back even when no image was found, so the
                  ;; initials-circle avatar fallback still renders for
                  ;; #Person (and the dashed placeholder for :image types).
                  (when-not @cancel-token
                    {:qid qid
                     :image-url image-url
