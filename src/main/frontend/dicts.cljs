...
(def dicts
  {:en {:tutorial/text (rc/inline "tutorial-en.md")
   ...
   :zh-CN {:on-boarding/title "你好，欢迎使用 Logseq！"
           ...
           :on-boarding/new-graph-desc-1 "Logseq 同时支持 Markdown 和 Org-mode，在设备上新建或者打开一个文件夹，数据将只保存在这台设备上"
           :on-boarding/new-graph-desc-2 "文件夹打开后，将会在里面创建 3 个目录:"
           :on-boarding/new-graph-desc-3 "/journals - 保存你的日志"
           :on-boarding/new-graph-desc-4 "/pages - 保存你的页面"
           :on-boarding/new-graph-desc-5 "/logseq - 保存配置，自定义 css 和一些元数据"
           :help/start "入门"
           :help/roadmap "路线图"
           ...
           :settings-page/edit-config-edn "编辑 config.edn"
           :settings-page/edit-custom-css "编辑 custom.css"
           :settings-page/git-desc "用于页面的版本管理，可以单击垂直的三个点菜单来查看页面的历史记录"
           ...
           :settings-page/enable-timetracking "开启时间跟踪"
