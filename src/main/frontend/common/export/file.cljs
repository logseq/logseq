(ns frontend.common.export.file
  ...
  (letfn [(entity-map [x]
          ...
          (node-ref-content [content]
            (if (and (:export-node-property-values-as-page-refs? context)
              ...
              (melange-common/to-page-ref content)
              content))]
    (cond
...
(defn- block-properties-content
  ...
  (let [block (or (when-let [id (:db/id block)]
                  ...
                  block)
        properties (->> (melange-property/properties block)
                        (remove (fn [[k _]]
                                  (contains? property-catalog/db-attribute-properties k)))
                        (remove (fn [[k _]]
    ...
    (when (seq properties)
      (let [sorted-properties (->> (keys properties)
                                   ...
                                   melange-property/sort-properties)]
        (->> sorted-properties
             (keep (fn [property]
...
(defn block->content
  ...
  (let [init-level (or (:init-level tree->file-opts)
        ...
        blocks (->> (d/pull-many db '[*] (keep :db/id (ldb/get-block-and-children db root-block-uuid)))
                    (map #(melange-content/update-block-content db % (:db/id %))))
