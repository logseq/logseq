(ns logseq.tasks.db.export-client-ops-ops
  "Export client-ops tx entities to an EDN file sorted by :db-sync/created-at asc.

  Examples:
  $ pnpm exec nbb-logseq src/logseq/tasks/db/export_client_ops_ops.cljs /path/to/client-ops.sqlite
  $ pnpm exec nbb-logseq src/logseq/tasks/db/export_client_ops_ops.cljs /path/to/client-ops.sqlite --out client-ops-ops.edn"
  (:require ["fs" :as fs]
            ["path" :as node-path]
            [babashka.cli :as cli]
...
(defn usage []
  (str "Usage: pnpm exec nbb-logseq src/logseq/tasks/db/export_client_ops_ops.cljs <client-ops-db> [--out <file.edn>]\n"
       "Options:\n"
