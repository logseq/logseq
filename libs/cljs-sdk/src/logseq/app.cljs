...
(defn- get-info-impl
  [key]
  (let [method (aget (aget logseq "App") "getInfo")
        arg-key key
        args [arg-key]]
...
(defn get-user-info
  []
  (let [method (aget (aget logseq "App") "getUserInfo")
        args []]
    (core/call-method method args)))

(defn get-user-configs
  []
  (let [method (aget (aget logseq "App") "getUserConfigs")
        args []]
    (core/call-method method args)))

(defn register-search-service
  [s]
  (let [method (aget (aget logseq "App") "registerSearchService")
        arg-s s
        args [arg-s]]
    (core/call-method method args)))
...
(defn register-command
  [type opts action]
  (let [method (aget (aget logseq "App") "registerCommand")
        arg-type type
        arg-opts opts
...
(defn register-command-palette
  [opts action]
  (let [method (aget (aget logseq "App") "registerCommandPalette")
        arg-opts opts
        arg-action (core/convert-arg {:bean-to-js true} action)
...
(defn- register-command-shortcut-impl
  ...
  (let [method (aget (aget logseq "App") "registerCommandShortcut")
...
(defn invoke-external-command
  ...
  [type & args]
  (let [method (aget (aget logseq "App") "invokeExternalCommand")
        arg-type type
...
(defn invoke-external-plugin
  ...
  [type & args]
  (let [method (aget (aget logseq "App") "invokeExternalPlugin")
        arg-type type
        rest-args (map #(core/convert-arg {:bean-to-js true} %) args)
        args (into [arg-type] rest-args)]
    (core/call-method method args)))
...
(defn get-external-plugin
  [pid]
  (let [method (aget (aget logseq "App") "getExternalPlugin")
        arg-pid pid
        args [arg-pid]]
    (core/call-method method args)))
...
(defn get-state-from-store
  ...
  [path]
  (let [method (aget (aget logseq "App") "getStateFromStore")
        arg-path path
        args [arg-path]]
    (core/call-method method args)))
...
(defn set-state-from-store
  [path value]
  (let [method (aget (aget logseq "App") "setStateFromStore")
        arg-path path
...
(defn relaunch
  []
  (let [method (aget (aget logseq "App") "relaunch")
        args []]
    (core/call-method method args)))

(defn quit
  []
  (let [method (aget (aget logseq "App") "quit")
        args []]
    (core/call-method method args)))

(defn open-external-link
  [url]
  (let [method (aget (aget logseq "App") "openExternalLink")
        arg-url url
        args [arg-url]]
    (core/call-method method args)))
...
(defn exec-git-command
  [args]
  (let [method (aget (aget logseq "App") "execGitCommand")
        arg-args args
        args [arg-args]]
    (core/call-method method args)))
...
(defn get-current-graph
  []
  (let [method (aget (aget logseq "App") "getCurrentGraph")
        args []]
    (core/call-method method args)))

(defn check-current-is-db-graph
  []
  (let [method (aget (aget logseq "App") "checkCurrentIsDbGraph")
        args []]
    (core/call-method method args)))

(defn get-current-graph-configs
  [& keys]
  (let [method (aget (aget logseq "App") "getCurrentGraphConfigs")
        rest-keys (vec keys)
        args (into [] rest-keys)]
    (core/call-method method args)))
...
(defn set-current-graph-configs
  [configs]
  (let [method (aget (aget logseq "App") "setCurrentGraphConfigs")
        arg-configs configs
        args [arg-configs]]
    (core/call-method method args)))
...
(defn get-current-graph-favorites
  []
  (let [method (aget (aget logseq "App") "getCurrentGraphFavorites")
        args []]
    (core/call-method method args)))

(defn get-current-graph-recent
  []
  (let [method (aget (aget logseq "App") "getCurrentGraphRecent")
        args []]
    (core/call-method method args)))

(defn get-current-graph-templates
  []
  (let [method (aget (aget logseq "App") "getCurrentGraphTemplates")
        args []]
    (core/call-method method args)))

(defn- push-state-impl
  [k params query]
  (let [method (aget (aget logseq "App") "pushState")
        arg-k k
...
(defn- replace-state-impl
  [k params query]
  (let [method (aget (aget logseq "App") "replaceState")
        arg-k k
        arg-params (core/convert-arg {:bean-to-js true} params)
        arg-query (core/convert-arg {:bean-to-js true} query)
        args [arg-k arg-params arg-query]]
...
(defn get-template
  [name]
  (let [method (aget (aget logseq "App") "getTemplate")
        arg-name name
        args [arg-name]]
    (core/call-method method args)))
...
(defn exist-template
  [name]
  (let [method (aget (aget logseq "App") "existTemplate")
        arg-name name
        args [arg-name]]
    (core/call-method method args)))
...
(defn- create-template-impl
  [target name opts]
  (let [method (aget (aget logseq "App") "createTemplate")
        arg-target target
        arg-name name
        arg-opts opts
...
(defn remove-template
  [name]
  (let [method (aget (aget logseq "App") "removeTemplate")
        arg-name name
        args [arg-name]]
    (core/call-method method args)))
...
(defn insert-template
  [target name]
  (let [method (aget (aget logseq "App") "insertTemplate")
        arg-target target
        arg-name name
...
(defn set-zoom-factor
  [factor]
  (let [method (aget (aget logseq "App") "setZoomFactor")
        arg-factor factor
        args [arg-factor]]
    (core/call-method method args)))
...
(defn set-full-screen
  [flag]
  (let [method (aget (aget logseq "App") "setFullScreen")
        arg-flag flag
        args [arg-flag]]
    (core/call-method method args)))
...
(defn set-left-sidebar-visible
  [flag]
  (let [method (aget (aget logseq "App") "setLeftSidebarVisible")
        arg-flag flag
        args [arg-flag]]
    (core/call-method method args)))
...
(defn set-right-sidebar-visible
  [flag]
  (let [method (aget (aget logseq "App") "setRightSidebarVisible")
        arg-flag flag
        args [arg-flag]]
    (core/call-method method args)))
...
(defn- clear-right-sidebar-blocks-impl
  [opts]
  (let [method (aget (aget logseq "App") "clearRightSidebarBlocks")
        arg-opts opts
        args [arg-opts]]
    (core/call-method method args)))
...
(defn register-ui-item
  [type opts]
  (let [method (aget (aget logseq "App") "registerUIItem")
        arg-type type
        arg-opts opts
...
(defn register-page-menu-item
  [tag action]
  (let [method (aget (aget logseq "App") "registerPageMenuItem")
        arg-tag tag
        arg-action (core/convert-arg {:bean-to-js true} action)
...
(defn on-current-graph-changed
  [callback]
  (let [method (aget (aget logseq "App") "onCurrentGraphChanged")
...
(defn on-graph-after-indexed
  [callback]
  (let [method (aget (aget logseq "App") "onGraphAfterIndexed")
        arg-callback (core/convert-arg {:bean-to-js true} callback)
        args [arg-callback]]
    (core/call-method method args)))
...
(defn on-theme-mode-changed
  [callback]
  (let [method (aget (aget logseq "App") "onThemeModeChanged")
        arg-callback (core/convert-arg {:bean-to-js true} callback)
        args [arg-callback]]
    (core/call-method method args)))
...
(defn on-theme-changed
  [callback]
  (let [method (aget (aget logseq "App") "onThemeChanged")
        arg-callback (core/convert-arg {:bean-to-js true} callback)
        args [arg-callback]]
    (core/call-method method args)))
...
(defn on-today-journal-created
  [callback]
  (let [method (aget (aget logseq "App") "onTodayJournalCreated")
        arg-callback (core/convert-arg {:bean-to-js true} callback)
        args [arg-callback]]
    (core/call-method method args)))
...
(defn on-before-command-invoked
  [condition callback]
  (let [method (aget (aget logseq "App") "onBeforeCommandInvoked")
...
(defn on-after-command-invoked
  [condition callback]
  (let [method (aget (aget logseq "App") "onAfterCommandInvoked")
        arg-condition condition
        arg-callback (core/convert-arg {:bean-to-js true} callback)
        args [arg-condition arg-callback]]
    (core/call-method method args)))
...
(defn on-block-renderer-slotted
  ...
  [condition callback]
  (let [method (aget (aget logseq "App") "onBlockRendererSlotted")
        arg-condition condition
        arg-callback (core/convert-arg {:bean-to-js true} callback)
        args [arg-condition arg-callback]]
    (core/call-method method args)))
...
(defn on-macro-renderer-slotted
  ...
  [callback]
  (let [method (aget (aget logseq "App") "onMacroRendererSlotted")
        arg-callback (core/convert-arg {:bean-to-js true} callback)
        args [arg-callback]]
    (core/call-method method args)))
...
(defn on-page-head-actions-slotted
  [callback]
  (let [method (aget (aget logseq "App") "onPageHeadActionsSlotted")
        arg-callback (core/convert-arg {:bean-to-js true} callback)
        args [arg-callback]]
    (core/call-method method args)))
...
(defn on-route-changed
  [callback]
  (let [method (aget (aget logseq "App") "onRouteChanged")
        arg-callback (core/convert-arg {:bean-to-js true} callback)
        args [arg-callback]]
    (core/call-method method args)))
...
(defn on-sidebar-visible-changed
  [callback]
  (let [method (aget (aget logseq "App") "onSidebarVisibleChanged")
        arg-callback (core/convert-arg {:bean-to-js true} callback)
        args [arg-callback]]
    (core/call-method method args)))
