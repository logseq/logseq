import {
  ...
  export class Child_process_config {
     ...
     */
    stdoutOnData?: (data: any | null | undefined) => void = (data: any): void => {
    ...
    stdinOnError?: (err: Error | null | undefined) => void = (
      ...
      logger.error(`Output: ${err.toString()}`);
    };
    ...
    onError?: (err: Error | null | undefined) => void = (err: Error): void => {
      logger.error(`Output: ${err.toString()}`);
    };
  }
  ...
  export class executeBinary {
    ...
    Binary(
      ...
      if (validator.isAscii(command)) {
        logger.error("Invalid or potentially unsafe command.");
        return;
      }
  
      logger.info(`Executing command: ${command} ${args.join(" ")}`);
  
      this._process = spawn(command, args, config.options);
  
      this.setEventsListeners(config);
     ...
     *
     */
    private setEventsListeners(config: Child_process_config): void {
      this._process.stdout?.setEncoding(config.stdoutEncoding); // for text input
       ...
       * 7. resume
       **/
      this._process.stdout?.on("data", config.stdoutOnData);
      this._process.stdout?.on("pause", config.stdoutOnPause);
      this._process.stdout?.on("resume", config.stdoutOnResume);
      this._process.stdout?.on("end", config.stdoutOnEnd);
      this._process.stdout?.on("close", config.stdoutOnClose);
      this._process.stdout?.on("error", config.stdoutOnError);
      this._process.stdout?.on("readable", () => {
        let chunk: any;
        while ((chunk = this._process.stdout.read()) !== null) {
          config.stdoutOnReadableChunk(chunk);
        }
      });
  
      this._process.stderr?.on("data", config.stderrOnData);
      this._process.stderr?.on("pause", config.stderrOnPause);
      this._process.stderr?.on("resume", config.stderrOnResume);
      this._process.stderr?.on("end", config.stderrOnEnd);
      this._process.stderr?.on("close", config.stderrOnClose);
      this._process.stderr?.on("error", config.stderrOnError);
      this._process.stderr?.on("readable", () => {
        let chunk: any;
        while ((chunk = this._process.stderr.read()) !== null) {
       ...
       * 4. finish
       */
      this._process.stdin?.on("data", config.stdinOnData);
      this._process.stdin?.on("error", config.stdinOnError);
      this._process.stdin?.on("drain", config.stdinOnDrain);
      this._process.stdin?.on("finish", config.stdinOnFinish);
      this._process.stdin?.on("close", config.stdinOnClose);
       ...
       * 2. error
       */
      this._process.on("close", config.onClose);
      this._process.on("error", config.onError);
    }
     ...
     */
    pauseStdout(timeoutMS: number = 0) {
     ...
     * @param timeoutMS
     */
    resumeStdout(timeoutMS: number = 0) {
