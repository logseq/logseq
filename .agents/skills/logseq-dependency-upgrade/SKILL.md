...
For npm packages with a range specifier (e.g. `^`), the script checks `pnpm-lock.yaml` to see if the resolved version already matches latest. These packages are flagged as **already resolved** — they need only a lockfile refresh, not a manifest change, and carry zero upgrade risk.
