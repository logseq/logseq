...
- **Escape inside a code editor** exits it into Logseq's normal block-edit mode — the block swaps to a raw `<textarea.multiline-block>` in EVERY position (main page + sidebar share the edit state). Click elsewhere to blur and the rendered editor returns. Don't mistake this for a broken render.
- Hover actions (`.code-block-actions`: language picker + Copy) sit top-right of `.ls-code-editor-wrap`. For calc blocks their right offset is `--logseq-code-actions-right`, which reserves the results column via `--logseq-code-calc-result-width` (measured from `.extensions__code-calc` and set on the wrap by the editor's rAF layout sync — `0px` for non-calc editors). Verify clearance objectively with `getBoundingClientRect()` on `.code-block-actions` vs `.extensions__code-calc`.
