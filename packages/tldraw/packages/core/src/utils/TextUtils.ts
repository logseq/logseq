...
export class TextUtils {
  ...
  static insert(field: HTMLTextAreaElement | HTMLInputElement, text: string): void {
    ...
    if (!document.execCommand('insertText', false, text)) {
      TextUtils.insertTextFirefox(field, text)
    }
  ...
  static wrapSelection(
    ...
    const { selectionStart, selectionEnd } = field
    const selection = TextUtils.getSelection(field)
    TextUtils.insert(field, wrap + selection + (wrapEnd ?? wrap))
  ...
  static replace(
    ...
    field.value.replace(searchValue, (...args): string => {
      ...
      TextUtils.insert(field, replacement)
  ...
  static indent(element: HTMLTextAreaElement): void {
    ...
    if (lineBreakCount && lineBreakCount > 0) {
      ...
      TextUtils.insert(element, indentedText)

      // Restore selection position, including the indentation
      element.setSelectionRange(selectionStart + 1, selectionEnd + replacementsCount)
    } else {
      TextUtils.insert(element, TextUtils.INDENT)
    }
  }
  ...
  static unindent(element: HTMLTextAreaElement): void {
    ...
    const firstLineStart = value.lastIndexOf('\n', selectionStart - 1) + 1
    const minimumSelectionEnd = TextUtils.findLineEnd(value, selectionEnd)
  ...
  static normalizeText(text: string) {
    return text.replace(TextUtils.fixNewLines, '\n')
  }
}
