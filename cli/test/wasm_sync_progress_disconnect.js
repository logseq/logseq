...
const root = fs.mkdtempSync(path.join(os.tmpdir(), "logseq-cli-wasm-sync-progress-disconnect-"));
...
function invokeResponse(body, res) {
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    if (req.method === "GET" && req.url === "/v1/events") {
      ...
      res.write(
        `data: {\"payload\":\"[\\\"~:rtc-log\\\",[\\\"^ \\\",\\\"~:type\\\",\\\"~:rtc.log/download\\\",\\\"~:graph-uuid\\\",\\\"~u${graphId}\\\",\\\"~:message\\\",\\\"download started\\\"]]\"}\n\n`
      );
    ...
    if (req.method === "POST" && req.url === "/v1/invoke") {
      try {
        const resultTransit = invokeResponse(body, res);
        if (resultTransit !== null) {
          res.writeHead(200, { "Content-Type": "application/json" });
