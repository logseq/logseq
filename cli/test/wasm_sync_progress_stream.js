...
const root = fs.mkdtempSync(path.join(os.tmpdir(), "logseq-cli-wasm-sync-progress-stream-"));
...
const largeSyncPayload = JSON.stringify([
  "~:sync-db-changes",
  [
    "^ ",
    "~:pages",
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    if (req.method === "GET" && req.url === "/v1/events") {
      ...
      res.write(
        `data: ${JSON.stringify({ type: "sync-db-changes", payload: largeSyncPayload })}\n\n`
      );
      res.write(
        `data: {\"payload\":\"[\\\"~:rtc-log\\\",[\\\"^ \\\",\\\"~:type\\\",\\\"~:rtc.log/download\\\",\\\"~:graph-uuid\\\",\\\"~u${graphId}\\\",\\\"~:message\\\",\\\"downloaded streaming block\\\"]]\"}\n\n`
      );
      return;
    }
...
server.listen(0, "127.0.0.1", () => {
  ...
  const timeout = setTimeout(() => {
    console.error("wasm sync download did not finish while events stream stayed open");
  ...
  child.on("exit", (code) => {
    ...
    if (downloadRequests !== 1) {
      console.error(`expected one download request while events stream stayed open, got ${downloadRequests}`);
      finish(1);
      return;
    }
    if (!stdout.includes("downloaded streaming block")) {
      console.error("expected streaming progress output");
      console.error(stdout);
      finish(1);
      return;
    }
