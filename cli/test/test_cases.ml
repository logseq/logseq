...
let () =
  test "--help prints usage and commands" (fun () ->
      ...
      expect_named_contains "help leaf command" output "graph create");
...
let () =
  ...
  test_promise "graph export edn accepts forwarded edn options" (fun () ->
      let root = temp_dir "logseq-cli-graph-export-edn-" in
      let export_path = Node.Path.join [| root; "export.edn" |] in
      let export_requests = ref 0 in
      let server =
        invoke_server (fun body ->
            if not (Js.String.includes ~search:"thread-api/export-edn" body)
            then fail_test ("unexpected invoke body: " ^ body);
            if not (Js.String.includes ~search:"logseq_db_alpha" body) then
              fail_test ("missing repo in request: " ^ body);
            if not (Js.String.includes ~search:"export-type" body) then
              fail_test ("missing export type in request: " ^ body);
            if not (Js.String.includes ~search:"graph-human" body) then
              fail_test ("missing forwarded export type in request: " ^ body);
            if not (Js.String.includes ~search:"graph-options" body) then
              fail_test ("missing forwarded graph options in request: " ^ body);
            if not (Js.String.includes ~search:"include-timestamps" body) then
              fail_test ("missing forwarded include-timestamps in request: " ^ body);
            incr export_requests;
            "[\"^ \",\"~:blocks\",[]]")
      in
      with_server server (fun base_url ->
          let* output =
          ...
          if output.code <> 0 then
            finish_with_error
              (Printf.sprintf
                 "expected CLI graph export to exit 0, got %d\n%s\n%s"
                 output.code output.stdout output.stderr)
          else if !export_requests <> 1 then
            finish_with_error
              (Printf.sprintf "expected one export request, got %d"
                 !export_requests)
          else if not (Node.Fs.existsSync export_path) then
            finish_with_error "expected export file"
          else if
            not
              (Js.String.includes ~search:"\"message\":\"wrote " output.stdout)
          then finish_with_error ("expected export result json\n" ^ output.stdout)
          else (
            ignore
              (expect_named_contains "export file" (read_file export_path)
                 ":blocks");
            remove_tree root;
            Js.Promise.resolve pass)));
