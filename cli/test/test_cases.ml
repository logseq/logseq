...
let unicode_text codepoints =
  Vec.string_concat ""
    (Vec.map Js.String.fromCodePoint (Vec.of_array codepoints))
...
let () =
  test "json output renders transit quoted uuid as string" (fun () ->
      ...
      let value =
        Edn_util.any
          (Melange_edn_melange.tagged "transit/quote" (Edn_util.uuid uuid))
      in
      let output =
...
let () =
  test "doctor resolves db-worker-node beside CLI entrypoint" (fun () ->
      ...
      try
        ...
        expect_named_contains "doctor entrypoint path" result##stdout
          worker_path
      with exn ->
        remove_tree root;
        remove_tree app_dir;
...
let () =
  ...
  test "graph commands require explicit unicode graph names" (fun () ->
      ...
      let run ?(env = [||]) args =
        spawn_cli ~env (Array.append [| "--root-dir"; root |] args)
      in
      try
        mkdir_p root;
        write_file
        ...
        let create_fallback =
          run [| "graph"; "create"; "-g"; fallback_graph |]
        in
        ...
        if not (Array.exists (( = ) encoded_graph_dir) graph_dirs) then
          fail_test
            (Printf.sprintf "graph dir: expected %s, got [%s]" encoded_graph_dir
               (graph_dirs |> Vec.of_array |> Vec.string_concat ", "));
  ...
  test "graph list ignores current-graph file" (fun () ->
      ...
      let run args = spawn_cli (Array.append [| "--root-dir"; root |] args) in
      try
        mkdir_p root;
        ignore
...
let () =
  ...
  test_promise "list page human output formats timestamps for large result sets"
    (fun () ->
      ...
      in
      let result_transit =
        "[" ^ Vec.string_concat "," (Vec.init 700 row) ^ "]"
      in
      let server =
        invoke_server (fun body ->
            incr request_count;
  ...
  test_promise "list asset human output keeps asset columns stable" (fun () ->
      ...
      with_server server (fun base_url ->
          ...
          if !request_count <> 2 then
            ...
            let expected_prefix = [| "id"; "title"; "size"; "type" |] in
            ...
            then
              fail_promise
                ...
                ^ (headers |> Vec.of_array |> Vec.string_concat ","))
            else if
              not
                (string_array_equal
  ...
  test_promise "list task human output uses registry header order" (fun () ->
      ...
      with_server server (fun base_url ->
          ...
          let headers =
            headers_from output.stdout |> Vec.of_array |> Vec.string_concat ","
          in
          ...
          then
            fail_promise
              ("task priority value kept namespace:\n" ^ output.stdout)
          else (
            ignore
  ...
  test_promise "upsert block update resolves tag names from tag list" (fun () ->
      ...
      let server =
        invoke_server (fun body ->
            ...
            | 2 when Js.String.includes ~search:"thread-api/cli-list-tags" body
              ->
          ...
          child_fields :=
            Vec.append !child_fields
              (Vec.of_array
                 ...
                 |]);
        "[[[\"^ \"," ^ Vec.string_concat "," !child_fields ^ "]]]"
      in
      let server =
        invoke_server (fun body ->
  ...
  run_sync_progress_case
    ...
    "logseq-cli-sync-progress-" ~delay_download_response:true
    (fun res ->
  ...
  test_promise
    ...
    (fun () ->
      ...
      let config_path = Node.Path.join [| root; "cli.edn" |] in
      let invoke_calls = ref Vec.empty in
      let method_names = ref Vec.empty in
      let record_method body method_name =
        if Js.String.includes ~search:method_name body then
          method_names := Vec.push_back !method_names method_name
      in
      let server =
        invoke_server (fun body ->
            invoke_calls := Vec.push_front !invoke_calls body;
