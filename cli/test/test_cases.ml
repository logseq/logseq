...
let () =
  test "--help prints usage and commands" (fun () ->
...
let () =
  ...
  test_promise "graph backup writes sqlite bytes and metadata" (fun () ->
      ...
      let server =
        invoke_server (fun body ->
            if
              not
                (Js.String.includes ~search:"thread-api/backup-db-sqlite" body)
            then fail_test ("unexpected invoke body: " ^ body);
            let snapshot_path = extract_snapshot_path body in
            let expected_root =
              Node.Path.join [| root; "graphs"; "alpha"; "backup" |]
            in
            if not (Js.String.includes ~search:expected_root snapshot_path) then
              fail_test ("unexpected snapshot path: " ^ snapshot_path);
      ...
      with_server server (fun base_url ->
          ...
          if output.code <> 0 then
            ...
            finish_with_error
              (Printf.sprintf "expected one backup request, got %d"
                 !backup_requests)
          else if
            not
          ...
          then
            ...
            if Array.length backups <> 1 then
              finish_with_error
                (Printf.sprintf "expected one backup directory, got %d"
                   (Array.length backups))
            else
              let backup_dir = Node.Path.join [| backup_root; backups.(0) |] in
              if
                read_file (Node.Path.join [| backup_dir; "db.sqlite" |])
                <> "sqlite-bytes"
              then finish_with_error "expected sqlite backup bytes"
              else if
                not
                  (Js.String.includes ~search:":source :cli"
                     (read_file
