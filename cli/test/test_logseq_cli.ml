...
let () =
        ...
        run_blocking
          (Cmdliner_boundary.eval app
             ~argv:
               ...
               |])
      with
      | Cmdliner_boundary.Parsed request -> (
          ...
          | Cli_request.Graph (Graph.Parsed_create opts) ->
              assert_bool ~name:"cmdliner eval enable sync" true
                opts.enable_sync
          | _ -> failf "expected graph create parsed command")
      | Cmdliner_boundary.Help help ->
          failf "expected parsed request, got help: %s" help
      | Cmdliner_boundary.Version version ->
          failf "expected parsed request, got version: %s" version
      | Cmdliner_boundary.Parse_error err ->
          failf "unexpected cmdliner eval parse error: %s" err.Error.message);

  check "Cmdliner_boundary.eval renders group help from registry" (fun () ->
      let app = Cmdliner_terms.app ~version:"test-version" () in
      match
        run_blocking
          (Cmdliner_boundary.eval app ~argv:[| "logseq"; "graph"; "--help" |])
      with
      | Cmdliner_boundary.Help help ->
          assert_contains ~name:"graph help includes graph list" "graph list"
            help;
          assert_contains ~name:"graph help includes backup restore"
            "graph backup restore" help;
          assert_not_contains ~name:"graph help excludes list page" "list page"
            help
      | Cmdliner_boundary.Parsed _ -> failf "expected help"
      | Cmdliner_boundary.Version version ->
          failf "expected help, got version: %s" version
      | Cmdliner_boundary.Parse_error err ->
          failf "unexpected help parse error: %s" err.Error.message);

  check "Cmdliner_boundary.eval returns parse errors for unknown commands"
    (fun () ->
      let app = Cmdliner_terms.app ~version:"test-version" () in
      match
        run_blocking
          (Cmdliner_boundary.eval app
             ~argv:[| "logseq"; "unknown"; "command" |])
      with
      | Cmdliner_boundary.Parse_error err ->
          assert_equal ~name:"unknown command code"
            (Edn_util.keyword_t "unknown-command")
            err.Error.code;
          assert_contains ~name:"unknown command message"
            "unknown command: unknown command" err.message
      | Cmdliner_boundary.Parsed request ->
          failf "expected parse error, got %s"
            (String.concat " " request.Cli_request.path)
      | Cmdliner_boundary.Help help ->
          failf "expected parse error, got help: %s" help
      | Cmdliner_boundary.Version version ->
      ...
      with_http_server
        (fun _request ->
          ( 400,
            ...
            {|{"error":{"code":"bad-request","message":"invalid graph"}}|} ))
        (fun base_url ->
          let message =
            try
              ignore
  ...
  check "Transport thread API wrapper posts to db-worker invoke endpoint"
    (fun () ->
      with_http_server
        (fun request ->
          assert_equal ~name:"invoke line" "POST /v1/invoke HTTP/1.1"
            request.request_line;
          assert_contains ~name:"invoke method" {|"method":"thread-api/q"|}
            request.body;
          assert_contains ~name:"invoke args"
            {|"argsTransit":"[\"logseq_db_alpha\",[\"~:block/title\"]]"|}
            request.body;
          ( 200,
            [ ("Content-Type", "application/json") ],
            {|{"resultTransit":"[\"^ \",\"~:ok\",true,\"~:items\",[]]"}|} ))
        (fun base_url ->
          let result =
            run_blocking
          ...
          | Some _ -> (
              (match Edn_util.get_bool result ":ok" with
              | Some true -> ()
              | _ -> failf "expected decoded ok true");
              match Edn_util.get result ":items" with
              | Some value when Edn_util.as_vector value = Some [] -> ()
      ...
      with_http_server
        (fun request ->
          assert_contains ~name:"escaped tilde string" {|\"~~literal\"|}
            request.body;
          assert_contains ~name:"canonical repo tilde string"
            {|\"logseq_db_~literal\"|} request.body;
          assert_contains ~name:"escaped cache string" {|\"~^literal\"|}
            request.body;
          assert_contains ~name:"escaped backtick string" {|\"~`literal\"|}
            request.body;
          ( 200,
            [ ("Content-Type", "application/json") ],
            {|{"resultTransit":"[]"}|} ))
            ...
            (run_blocking
               (Transport.thread_api_apply_outliner_ops
                  ...
                  ~options:
                    (Edn_util.map_t
                       [
                         (Edn_util.keyword ":value", Edn_util.string "`literal");
    ...
    (fun () ->
      with_http_server
        (fun _request ->
          ( 200,
            [ ("Content-Type", "application/json") ],
            {|{"resultTransit":"[\"~~literal\",\"~^literal\",\"~`literal\"]"}|}
          ))
        (fun base_url ->
          let result =
            run_blocking
          ...
          | Some [ first; second; third ]
            when Edn_util.as_string first = Some "~literal"
                 && Edn_util.as_string second = Some "^literal"
                 && Edn_util.as_string third = Some "`literal" ->
              ()
          | _ ->
              failf
    ...
    (fun () ->
      with_http_server
        (fun _request ->
          ( 200,
            [ ("Content-Type", "application/json") ],
            {|{"resultTransit":"[\"~#list\",[[\"^ \",\"~:db/id\",1,\"~:block/title\",\"Home\",\"~:logseq.property/type\",\"~:page\"],[\"^ \",\"^1\",2,\"^2\",\"About\",\"^3\",\"^4\"]]]"}|}
          ))
        (fun base_url ->
          let result =
            run_blocking
          ...
          match Edn_util.as_seq result with
          | Some [ first; second ]
            when Option.is_some (Edn_util.as_map first)
                 && Option.is_some (Edn_util.as_map second) -> (
              (match Edn_util.get_string first ":block/title" with
              | Some "Home" -> ()
              | _ -> failf "expected first title");
              (match Edn_util.get_int64 second ":db/id" with
              | Some 2L -> ()
              | _ -> failf "expected second id");
              match Edn_util.get_string second ":block/title" with
              | Some "About" -> ()
  ...
  check "Transport thread API wrapper decodes Transit base-44 cache codes" (fun () ->
      with_http_server
        (fun _request ->
          ( 200,
            [ ("Content-Type", "application/json") ],
            {|{"resultTransit":"[\"~#list\",[[\"^ \",\"~:k1\",1,\"~:k2\",2,\"~:k3\",3,\"~:k4\",4,\"~:k5\",5,\"~:k6\",6,\"~:k7\",7,\"~:k8\",8,\"~:k9\",9,\"~:k10\",10],[\"^ \",\"^:\",20]]]"}|}
          ))
        (fun base_url ->
          let result =
            run_blocking
          ...
          | Some [ _; second ] when Option.is_some (Edn_util.as_map second) -> (
              match Edn_util.get_int second ":k10" with
              | Some 20 -> ()
  ...
  check "Transport thread API wrapper decodes Transit JSON object map keys" (fun () ->
      with_http_server
        ...
        (fun base_url ->
          ...
          match Edn_util.as_map result with
          | Some _ -> (
              (match Edn_util.get_int64 result ":db/id" with
              | Some 1L -> ()
              | _ -> failf "expected object map db id");
              match Edn_util.get_string result ":block/title" with
              | Some "Home" -> ()
    ...
    (fun () ->
      with_http_server
        (fun _request ->
          ( 200,
            [ ("Content-Type", "application/json") ],
            {|{"resultTransit":"[\"^ \",\"~:block/name\",\"page\",\"~:block/tags\",[[\"^ \",\"^0\",\"tag\",\"~:block/title\",\"Tag\",\"~:block/uuid\",\"~u00000002-5389-0208-3000-000000000000\",\"~:db/id\",2,\"~:db/ident\",\"~:logseq.class/Tag\"]],\"^2\",\"Page\",\"^3\",\"~u00000002-1484-3403-2900-000000000000\",\"^4\",4,\"^5\",\"~:logseq.class/Page\"]"}|}
          ))
        (fun base_url ->
          let result =
            run_blocking
          ...
          | Some _ -> (
              (match Edn_util.get_string result ":block/title" with
              | Some "Page" -> ()
              | _ -> failf "expected pull title");
              (match Edn_util.get_int64 result ":db/id" with
              | Some 4L -> ()
              | _ -> failf "expected pull db id");
              match Edn_util.get result ":block/uuid" with
              | Some value
                when Edn_util.as_uuid value
                     = Some "00000002-1484-3403-2900-000000000000" ->
                  ()
  ...
  check "Transport thread API wrapper decodes verbose Transit composites" (fun () ->
      with_http_server
        ...
        (fun base_url ->
          ...
          match Edn_util.as_seq result with
          | Some [ first; second ]
            when Edn_util.as_string first = Some "Home"
                 && Edn_util.as_string second = Some "About" ->
              ()
          | _ -> failf "expected verbose Transit list to decode as a sequence"));

  check "Transport.connect_events decodes SSE Transit payload events" (fun () ->
      let events = ref [] in
      with_http_server
        (fun request ->
          assert_equal ~name:"events request line" "GET /v1/events HTTP/1.1"
            request.request_line;
          (match List.assoc_opt "accept" request.headers with
          | Some "text/event-stream" -> ()
          | Some value -> failf "unexpected events accept header: %s" value
          | None -> failf "missing events accept header");
          ( 200,
            [ ("Content-Type", "text/event-stream") ],
            String.concat ""
              [
                "data: {\"payload\":\"[\\\"~:sync/progress\\\",[\\\"^ \
                 \\\",\\\"~:done\\\",true]]\"}\n\n";
                "data: {\"type\":\"agent/message\",\"payload\":\"[\\\"^ \
                 \\\",\\\"~:text\\\",\\\"hi\\\"]\"}\n\n";
              ] ))
        (fun base_url ->
          let subscription =
            run_blocking
              (Transport.connect_events
                 ...
                 (fun event_type payload ->
                   ...
                   Cli_effect.pure ()))
          in
          if not (wait_until 100 (fun () -> List.length !events = 2)) then
            failf "expected two decoded SSE events";
          run_blocking (subscription.Transport.close ());
          match !events with
          | [ (progress_type, progress); (message_type, message) ]
            when progress_type = Edn_util.keyword_t ":sync/progress"
                 && message_type = Edn_util.keyword_t ":agent/message"
                 && Option.is_some (Edn_util.as_map progress)
                 && Option.is_some (Edn_util.as_map message) -> (
              (match Edn_util.get_bool progress ":done" with
              | Some true -> ()
              | _ -> failf "expected progress done flag");
              match Edn_util.get_string message ":text" with
              | Some "hi" -> ()
              | _ -> failf "expected message text")
          | _ -> failf "expected decoded SSE event list"));

  check "Transport.write_output and read_input round trip EDN values" (fun () ->
      let path = Filename.temp_file "logseq-cli-transport-edn-" ".edn" in
      let data =
        Edn_util.map
          [
            (Edn_util.keyword ":db/id", Edn_util.int 42);
            (Edn_util.keyword ":block/title", Edn_util.string "Home");
            ( Edn_util.keyword ":block/tags",
              Edn_util.vector [ Edn_util.keyword ":logseq.class/Page" ] );
          ]
      in
      (match
         run_blocking
           (Transport.write_output
              ~format:(Edn_util.keyword_t ":edn")
              ~path ~data)
       with
      | Ok () -> ()
      | Error err -> failf "unexpected write edn error: %s" err.Error.message);
      match
        run_blocking
          (Transport.read_input ~format:(Edn_util.keyword_t ":edn") ~path)
      with
      | Ok value when Option.is_some (Edn_util.as_map value) -> (
          (match Edn_util.get_int64 value ":db/id" with
          | Some 42L -> ()
          | _ -> failf "expected EDN db id");
          (match Edn_util.get_string value ":block/title" with
          | Some "Home" -> ()
          | _ -> failf "expected EDN title");
          match Edn_util.get value ":block/tags" with
          | Some tags -> (
              match Edn_util.as_vector tags with
              | Some [ tag ]
                when Edn_util.as_keyword tag = Some ":logseq.class/Page" ->
                  ()
              | _ -> failf "expected EDN tags")
          | _ -> failf "expected EDN tags")
      | Ok other ->
          failf "expected EDN map, got %s" (Edn_ocaml.to_edn_string other)
      | Error err -> failf "unexpected read edn error: %s" err.Error.message);

  check "Transport.read_input parses EDN discard forms" (fun () ->
      let path =
        Filename.temp_file "logseq-cli-transport-edn-discard-" ".edn"
      in
      write_text_file path {|[:keep #_[:discarded] :done]|};
      match
        run_blocking
          (Transport.read_input ~format:(Edn_util.keyword_t ":edn") ~path)
      with
      | Ok value -> (
          ...
          | Some [ keep; done_ ]
            when Edn_util.as_keyword keep = Some ":keep"
                 && Edn_util.as_keyword done_ = Some ":done" ->
              ()
          | _ ->
              failf "unexpected EDN discard result: %s"
                (Edn_ocaml.to_edn_string value))
      | Error err -> failf "unexpected EDN discard error: %s" err.Error.message);

  check "Transport.write_output and read_input preserve db binary data"
    (fun () ->
      let path = Filename.temp_file "logseq-cli-transport-db-" ".db" in
      let bytes = Bytes.of_string "\000logseq\255db" in
      (match
         run_blocking
           (Transport.write_output ~format:(Edn_util.keyword_t ":db") ~path
              ~data:(Edn_util.bytes bytes))
       with
      | Ok () -> ()
      | Error err -> failf "unexpected write db error: %s" err.Error.message);
      assert_equal ~name:"written db bytes" (Bytes.to_string bytes)
        (read_binary_file path);
      match
        run_blocking
          (Transport.read_input ~format:(Edn_util.keyword_t ":db") ~path)
      with
      | Ok value when Option.is_some (Edn_util.as_bytes value) ->
          let actual =
            Option.value (Edn_util.as_bytes value) ~default:(Bytes.create 0)
          in
          assert_equal ~name:"read db bytes" (Bytes.to_string bytes)
            (Bytes.to_string actual)
      | Ok other ->
          failf "expected db bytes, got %s" (Edn_ocaml.to_edn_string other)
      | Error err -> failf "unexpected read db error: %s" err.Error.message);

  check "Transport.read_input rejects unsupported formats" (fun () ->
      let path = Filename.temp_file "logseq-cli-transport-json-" ".json" in
      write_text_file path "{}";
      match
        run_blocking
          (Transport.read_input ~format:(Edn_util.keyword_t ":json") ~path)
      with
      | Error err ->
          assert_equal ~name:"unsupported input code"
            (Edn_util.keyword_t "unsupported-input-format")
            err.Error.code;
          assert_contains ~name:"unsupported input message"
            "unsupported input format: :json" err.message
      | Ok _ -> failf "expected unsupported input format error");

  check "Sha256.base64url matches PKCE challenge vectors" (fun () ->
      assert_equal ~name:"rfc pkce challenge"
        "E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM"
        (Sha256.base64url "dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk");
      assert_equal ~name:"login verifier challenge"
        "xTYRBLTDt1gS3-rw_3FBVafZ0iNg9bZYBbG8fRPvkEs"
        (Sha256.base64url "verifier-1"));

  check "Uuid_refs_types extracts and replaces uuid block refs" (fun () ->
      let first = "11111111-1111-1111-1111-111111111111" in
      let second = "22222222-2222-2222-2222-222222222222" in
      assert_equal ~name:"uuid refs extracted"
        (String.concat "," [ first; second ])
        (String.concat ","
           (Uuid_refs_types.extract_uuid_refs
              ("Start [["
              ^ String.uppercase_ascii first
              ^ "]] repeat [[" ^ first ^ "]] invalid [[not-a-uuid]] [[" ^ second
              ^ "]]")));
      assert_equal ~name:"uuid refs replaced" "See [[Project]] and [[Task]]"
        (Uuid_refs_types.replace_uuid_refs
           ("See [[" ^ first ^ "]] and [[" ^ second ^ "]]")
           [ (first, "Project"); (second, "Task") ]));

  check "Uuid_refs_types normalizes entity string fields" (fun () ->
      ...
      let item =
        Entity.of_value
          (value_with
             [
               (":db/id", vint 1);
               (":block/title", vstr ("Links [[" ^ uuid ^ "]]"));
             ])
      in
      match
        Uuid_refs_types.normalize_item_string_fields [ item ]
          [ Edn_util.keyword_t ":block/title" ]
          [ (uuid, "Home") ]
      with
      | [ normalized ] -> (
          match Edn_util.get_string normalized.Entity.raw ":block/title" with
          | Some "Links [[Home]]" -> ()
          | Some value -> failf "expected normalized title, got %S" value
          | None -> failf "expected normalized title")
      | _ -> failf "expected one normalized item");

  check "Log_types.truncate_preview follows CLI preview semantics" (fun () ->
      let short =
        Log_types.truncate_preview (Edn_util.string "abcdef") ~max_len:3
      in
      assert_equal ~name:"short preview text" "\"ab" short.preview;
      assert_int ~name:"short preview length" 8 short.length;
      assert_bool ~name:"short preview truncated" true short.truncated;
      let default_text = String.make 401 'x' in
      let default_preview =
        Log_types.truncate_preview (Edn_util.string default_text)
      in
      assert_int ~name:"default preview length" 403 default_preview.length;
      assert_int ~name:"default preview max" 400
        (String.length default_preview.preview);
      assert_bool ~name:"default preview truncated" true
        default_preview.truncated;
      let zero =
        Log_types.truncate_preview (Edn_util.string "abc") ~max_len:(-1)
      in
      assert_equal ~name:"negative max preview" "" zero.preview;
      assert_int ~name:"negative max length" 5 zero.length;
      assert_bool ~name:"negative max truncated" true zero.truncated);

  check "Log_types writes verbose records to stderr by level" (fun () ->
      let _, info_output =
        capture_stderr (fun () ->
            Log_types.set_verbose false;
            Log_types.log Log_types.Debug "hidden"
              (Some (Edn_util.string "secret"));
            Log_types.log Info "visible"
              (Some (Edn_util.map [ (Edn_util.keyword ":a", Edn_util.int 1) ])))
      in
      assert_not_contains ~name:"debug hidden" "hidden" info_output;
      assert_contains ~name:"info visible level" ":level :info" info_output;
      assert_contains ~name:"info visible message" ":message visible"
        info_output;
      assert_contains ~name:"info visible preview" ":preview {:a 1}" info_output;
      let _, debug_output =
        capture_stderr (fun () ->
            Log_types.set_verbose true;
            Log_types.log Debug "shown" (Some (Edn_util.string "payload"));
            Log_types.set_verbose false)
      in
      assert_contains ~name:"debug shown level" ":level :debug" debug_output;
      assert_contains ~name:"debug shown message" ":message shown" debug_output;
      assert_contains ~name:"debug shown preview" ":preview \"payload\""
        debug_output);

  check "Uuid_refs_types fetches uuid labels through db-worker" (fun () ->
      let uuid = "11111111-1111-1111-1111-111111111111" in
      with_http_server
        (fun request ->
          assert_equal ~name:"uuid label pull line" "POST /v1/invoke HTTP/1.1"
            request.request_line;
          assert_contains ~name:"uuid label pull method"
            {|"method":"thread-api/pull"|} request.body;
          assert_contains ~name:"uuid label lookup attr" {|\"~:block/uuid\"|}
            request.body;
          assert_contains ~name:"uuid label lookup value"
            {|~u11111111-1111-1111-1111-111111111111|} request.body;
          ( 200,
            [ ("Content-Type", "application/json") ],
            {|{"resultTransit":"[\"^ \",\"~:db/id\",7,\"~:block/uuid\",\"~u11111111-1111-1111-1111-111111111111\",\"~:block/title\",\"Home\"]"}|}
          ))
        (fun base_url ->
          let config =
            test_config
              ~root_dir:(fresh_root "uuid-label-root")
              ~base_url ~graph:"alpha" ()
          in
          match
            run_blocking
              (Uuid_refs_types.fetch_uuid_labels config (repo "logseq_db_alpha")
                 [ uuid; uuid; "not-a-uuid" ])
          with
          | [ (fetched_uuid, "Home") ] ->
              assert_equal ~name:"uuid label key" uuid fetched_uuid
          | labels ->
              failf "expected one uuid label, got %d" (List.length labels)));

  check "Add parses tag and property EDN options" (fun () ->
      (match
         Add.parse_tags_option
           (Some
              {|["Project" :logseq.class/Task 42 "11111111-1111-1111-1111-111111111111"]|})
       with
      | Ok
          [
            Selector.Tag_name "Project";
            Tag_ident tag_ident;
            Tag_id 42L;
            Tag_uuid "11111111-1111-1111-1111-111111111111";
          ]
        when tag_ident = Edn_util.keyword_t ":logseq.class/Task" ->
          ()
      | Ok _ -> failf "unexpected parsed tags"
      | Error err -> failf "unexpected tags parse error: %s" err.Error.message);
      (match Add.parse_tags_option (Some "[]") with
      | Error err ->
          assert_equal ~name:"empty tags code"
            (Edn_util.keyword_t "invalid-options")
            err.Error.code;
          assert_equal ~name:"empty tags message"
            "tags must be a non-empty vector" err.message
      | Ok _ -> failf "expected empty tags to fail");
      (match
         Add.parse_properties_option ~allow_non_built_in:true
           (Some {|{:logseq.property/owner "Alice" "custom key" 7 99 true}|})
       with
      | Ok
          [
            { Property.key = Property.Key_ident owner_key; value = owner };
            { key = Key_ident custom_key; value = custom };
            { key = Key_id 99L; value = enabled };
          ]
        when owner_key = Edn_util.keyword_t ":logseq.property/owner"
             && custom_key = Edn_util.keyword_t ":custom key"
             && Edn_util.as_string owner = Some "Alice"
             && Edn_util.as_int custom = Some 7
             && Edn_util.as_bool enabled = Some true ->
          ()
      | Ok _ -> failf "unexpected parsed properties"
      | Error err ->
          failf "unexpected properties parse error: %s" err.Error.message);
      match
        Add.parse_properties_vector_option ~allow_non_built_in:true
          (Some {|[:logseq.property/owner "custom key" 99]|})
      with
      | Ok [ Property.Key_ident owner; Key_ident custom; Key_id 99L ]
        when owner = Edn_util.keyword_t ":logseq.property/owner"
             && custom = Edn_util.keyword_t ":custom key" ->
          ()
      ...
      let opts =
        {
          ...
          properties_edn = Some {|{:logseq.property/owner "Alice"}|};
          content = Some "New block";
          blocks_edn = None;
          blocks_file = None;
        }
      in
      match Add.build_add_block_action opts [] (repo "logseq_db_alpha") with
      | Ok action -> (
          (match action.Add.tags with
          | [ Selector.Tag_name "Project" ] -> ()
          | _ -> failf "expected parsed add tags");
          (match action.properties with
          | [ { Property.key = Property.Key_ident key; value } ]
            when key = Edn_util.keyword_t ":logseq.property/owner"
                 && Edn_util.as_string value = Some "Alice" ->
              ()
          | _ -> failf "expected parsed add properties");
          match action.status with
          | Some status
            when status = Edn_util.keyword_t ":logseq.property/status.todo" ->
              ()
          | _ -> failf "expected normalized add status")
      | Error err -> failf "unexpected add action error: %s" err.Error.message);

  check "Add.build_add_block_action rejects invalid target uuid" (fun () ->
      let opts =
        {
          Add.target_id = None;
          target_uuid = Some "not-a-uuid";
      ...
      match Add.build_add_block_action opts [] (repo "logseq_db_alpha") with
      | Error err ->
          assert_equal ~name:"add invalid target uuid code"
            (Edn_util.keyword_t "invalid-options")
            err.Error.code;
          assert_equal ~name:"add invalid target uuid message"
            "Option target-uuid must be a valid UUID string" err.message
      | Ok _ -> failf "expected invalid add target uuid");

  check
    "Add.build_add_block_action uses positional command args as content \
     fallback" (fun () ->
      let opts =
        {
          Add.target_id = None;
      ...
      | Ok action -> (
          ...
          | [ block ] ->
              assert_opt_string ~name:"add args content title"
                (Some "Ship release") block.Block.title
          | _ -> failf "expected one block from positional args")
      | Error err ->
          failf "unexpected add args content error: %s" err.Error.message);

  check
    "Add.build_add_block_action lets explicit content override positional args"
    (fun () ->
      let opts =
        {
          Add.target_id = None;
      ...
      | Ok action -> (
          match action.blocks with
          | [ block ] ->
              assert_opt_string ~name:"add explicit content title"
                (Some "Explicit title") block.Block.title
          | _ -> failf "expected one block from explicit content")
      | Error err ->
          failf "unexpected add explicit content error: %s" err.Error.message);

  check "Add.build_add_block_action rejects invalid status" (fun () ->
      let opts =
        {
          Add.target_id = None;
          target_uuid = None;
      ...
      match Add.build_add_block_action opts [] (repo "logseq_db_alpha") with
      | Error err ->
          assert_equal ~name:"add invalid status code"
            (Edn_util.keyword_t "invalid-options")
            err.Error.code;
          assert_equal ~name:"add invalid status message"
            "invalid status: blocked" err.message
      | Ok _ -> failf "expected invalid add status");

  check "Add.invalid_options rejects block vectors with direct metadata"
    (fun () ->
      let opts =
        {
          Add.target_id = None;
          ...
          properties_edn = None;
          content = None;
          blocks_edn = Some {|[{:block/title "New block"}]|};
          blocks_file = None;
        }
      in
      assert_opt_string ~name:"add blocks tags conflict"
        (Some
           "tags and properties cannot be combined with --blocks or \
            --blocks-file") (Add.invalid_options opts);
      assert_opt_string ~name:"add blocks properties conflict"
        (Some
           "tags and properties cannot be combined with --blocks or \
            --blocks-file")
        (Add.invalid_options
  ...
  check "Add.resolve_tags resolves tag names through db-worker" (fun () ->
      let config =
        test_config
          ~root_dir:(fresh_root "logseq-cli-add-resolve-tags-")
          ~base_url:"http://127.0.0.1:0" ~graph:"alpha" ()
      in
      with_http_server_sequence
        [
          (fun request ->
            assert_equal ~name:"add resolve tag request line"
              "POST /v1/invoke HTTP/1.1" request.request_line;
            assert_contains ~name:"add resolve tag method"
              {|"method":"thread-api/q"|} request.body;
            assert_contains ~name:"add resolve tag name" {|project|}
              request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
              {|{"resultTransit":"[[\"^ \",\"~:db/id\",42,\"~:block/title\",\"Project\",\"~:block/name\",\"project\",\"~:block/uuid\",\"~u11111111-1111-1111-1111-111111111111\",\"~:block/tags\",[[\"^ \",\"~:db/ident\",\"~:logseq.class/Tag\"]]]]"}|}
            ));
        ]
        (fun base_url ->
          let config = { config with Cli_config.base_url = Some base_url } in
          match
            run_blocking
              (Add.resolve_tags config (repo "logseq_db_alpha")
                 [ Selector.Tag_name "project" ])
          with
          | [ tag ] ->
              assert_opt_int64 ~name:"resolved tag id" (Some 42L) tag.Entity.id;
              assert_opt_string ~name:"resolved tag title" (Some "Project")
                tag.title
          | _ -> failf "expected one resolved tag"));

  check "Add.resolve_properties resolves named properties through db-worker"
    (fun () ->
      let config =
        test_config
          ~root_dir:(fresh_root "logseq-cli-add-resolve-properties-")
          ~base_url:"http://127.0.0.1:0" ~graph:"alpha" ()
      in
      with_http_server_sequence
        [
          (fun request ->
            assert_equal ~name:"add resolve property request line"
              "POST /v1/invoke HTTP/1.1" request.request_line;
            assert_contains ~name:"add resolve property method"
              {|"method":"thread-api/q"|} request.body;
            assert_contains ~name:"add resolve property name" {|owner|}
              request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
              {|{"resultTransit":"[[\"^ \",\"~:db/id\",9,\"~:db/ident\",\"~:owner\",\"~:block/title\",\"Owner\",\"~:block/name\",\"owner\",\"~:logseq.property/type\",\"~:default\"]]"}|}
            ));
        ]
        (fun base_url ->
          let config = { config with Cli_config.base_url = Some base_url } in
          let assignments =
            run_blocking
              (Add.resolve_properties config (repo "logseq_db_alpha")
                 [
                   {
                     Property.key = Property.Key_name "owner";
                     value = Edn_util.string "Alice";
                   };
                 ])
          in
          match assignments with
          | [ { Property.key = Property.Key_ident key; value } ]
            when key = Edn_util.keyword_t ":owner"
                 && Edn_util.as_string value = Some "Alice" ->
              ()
          | _ -> failf "expected resolved property assignment"));

  check "Add.execute_add_block applies action metadata to inserted blocks"
    (fun () ->
      with_http_server_sequence
        [
          (fun request ->
            assert_equal ~name:"add metadata target page q line"
              "POST /v1/invoke HTTP/1.1" request.request_line;
            assert_contains ~name:"add metadata target page q method"
              {|"method":"thread-api/q"|} request.body;
            assert_contains ~name:"add metadata target page q name" {|home|}
              request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
          ...
          (fun request ->
            assert_equal ~name:"add metadata resolve tag line"
              "POST /v1/invoke HTTP/1.1" request.request_line;
            assert_contains ~name:"add metadata resolve tag method"
              {|"method":"thread-api/q"|} request.body;
            assert_contains ~name:"add metadata resolve tag name" {|project|}
              request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
          ...
          (fun request ->
            assert_equal ~name:"add metadata resolve property line"
              "POST /v1/invoke HTTP/1.1" request.request_line;
            assert_contains ~name:"add metadata resolve property method"
              {|"method":"thread-api/pull"|} request.body;
            assert_contains ~name:"add metadata resolve property ident"
              {|\"~:logseq.property/owner\"|} request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
              {|{"resultTransit":"[\"^ \",\"~:db/id\",7,\"~:db/ident\",\"~:logseq.property/owner\",\"~:logseq.property/type\",\"~:default\"]"}|}
            ));
          (fun request ->
            assert_equal ~name:"add metadata apply line"
              "POST /v1/invoke HTTP/1.1" request.request_line;
            assert_contains ~name:"add metadata apply method"
              {|"method":"thread-api/apply-outliner-ops"|} request.body;
            assert_contains ~name:"add metadata insert op"
              {|\"~:insert-blocks\"|} request.body;
            assert_contains ~name:"add metadata block title" {|New block|}
              request.body;
            assert_contains ~name:"add metadata batch set op"
              {|\"~:batch-set-property\"|} request.body;
            assert_contains ~name:"add metadata block uuid"
              {|~u33333333-3333-3333-3333-333333333333|} request.body;
            assert_contains ~name:"add metadata status property"
              {|\"~:logseq.property/status\"|} request.body;
            assert_contains ~name:"add metadata status value"
              {|\"~:logseq.property/status.todo\"|} request.body;
            assert_contains ~name:"add metadata tags property"
              {|\"~:block/tags\"|} request.body;
            assert_contains ~name:"add metadata tag id" {|,6,|} request.body;
            assert_contains ~name:"add metadata owner property"
              {|\"~:logseq.property/owner\"|} request.body;
            assert_contains ~name:"add metadata owner value" {|Alice|}
              request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
          ...
          (fun request ->
            assert_equal ~name:"add metadata pull created line"
              "POST /v1/invoke HTTP/1.1" request.request_line;
            assert_contains ~name:"add metadata pull created method"
              {|"method":"thread-api/pull"|} request.body;
            assert_contains ~name:"add metadata pull created uuid"
              {|~u33333333-3333-3333-3333-333333333333|} request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
        ...
        (fun base_url ->
          let config =
            test_config
              ~root_dir:(fresh_root "logseq-cli-add-metadata-")
              ~base_url ~graph:"alpha" ()
          in
          let action =
            {
              Add.repo = repo "logseq_db_alpha";
              graph = graph "alpha";
              target_id = None;
              target_uuid = None;
              target_page_name = Some "Home";
              pos = Block.Last_child;
              status = Some (Edn_util.keyword_t ":logseq.property/status.todo");
              tags = [ Selector.Tag_name "Project" ];
              properties =
                [
                  {
                    Property.key =
                      Property.Key_ident
                        (Edn_util.keyword_t ":logseq.property/owner");
                    value = Edn_util.string "Alice";
                  };
                ];
              blocks =
                [
                  Block.make ~uuid:"33333333-3333-3333-3333-333333333333"
                    ~title:"New block" ();
                ];
            }
          in
          let result =
            run_blocking (Add.execute_add_block action config Output.Mode.Json)
          in
          assert_int ~name:"add metadata exit code" 0
            (Cli_result.exit_code result);
          match result.Cli_result.data with
          | Some (Cli_result.Raw value)
            when Option.is_some (Edn_util.as_map value) -> (
              let fields = Option.value (Edn_util.as_map value) ~default:[] in
              match List.assoc_opt (Edn_util.keyword ":result") fields with
              | Some result -> (
                  match Edn_util.as_vector result with
                  | Some [ id ] when Edn_util.as_int64 id = Some 8L -> ()
                  | _ -> failf "expected created id result")
              | _ -> failf "expected created id result")
          | _ -> failf "expected raw add result"));

  check "Update.invalid_options validates block update options" (fun () ->
      let base =
        {
          Update.id = Some 7L;
          uuid = None;
      ...
      in
      assert_opt_string ~name:"valid update options" None
        (Update.invalid_options base);
      assert_opt_string ~name:"pos requires target"
        (Some "--pos is only valid when a target option is provided")
        (Update.invalid_options { base with pos = Some Block.Last_child });
      assert_opt_string ~name:"blocks create-only"
        (Some "--blocks and --blocks-file are only for create mode")
        (Update.invalid_options { base with blocks_edn = Some "[]" });
      assert_opt_string ~name:"single source selector"
        (Some "only one of --id or --uuid is allowed")
        (Update.invalid_options
           { base with uuid = Some "11111111-1111-1111-1111-111111111111" });
      assert_opt_string ~name:"single target selector"
        (Some
           "only one of --target-id, --target-uuid, or --target-page is allowed")
        (Update.invalid_options
           { base with target_id = Some 8L; target_page = Some "Home" }));

  check "Update.build_action parses selectors updates and metadata" (fun () ->
      let opts =
        {
          Update.id = Some 7L;
          uuid = None;
          ...
          update_properties_edn = Some {|{:owner "me"}|};
          remove_tags_edn = Some {|["old"]|};
          remove_properties_edn = Some {|[:stale]|};
          blocks_edn = None;
          blocks_file = None;
        }
      in
      match Update.build_action opts (repo "logseq_db_alpha") with
      | Error err -> failf "unexpected update build error: %s" err.Error.message
      | Ok action ->
          assert_equal ~name:"update repo" "logseq_db_alpha"
            (repo_string action.repo);
          assert_equal ~name:"update graph" "alpha" (graph_string action.graph);
          assert_int64 ~name:"update source id" 7L
            (Option.value action.id ~default:0L);
          assert_equal ~name:"update target label" "page:Home"
            (Option.value action.target_label ~default:"");
          assert_equal ~name:"update source label" "7"
            (Option.value action.source_label ~default:"");
          assert_equal ~name:"update default pos" "first-child"
            (Option.map Block.string_of_position action.pos
            |> Option.value ~default:"");
          assert_int ~name:"update tags count" 1
            (List.length action.update_tags);
          assert_int ~name:"remove tags count" 1
            (List.length action.remove_tags);
          assert_int ~name:"update property count includes status" 2
            (List.length action.update_properties);
          assert_int ~name:"remove property count" 1
            (List.length action.remove_properties);
          assert_equal ~name:"update content" "Updated"
            (Option.value action.content ~default:"");
          let metadata = Update.metadata () in
          assert_int ~name:"update metadata count" 1 (List.length metadata);
          let meta =
            match metadata with
            | [ meta ] -> meta
            | _ -> failf "expected one update metadata entry"
          in
          assert_equal ~name:"update metadata path" "upsert block"
            (String.concat " " meta.path);
          assert_equal ~name:"update metadata doc" "Upsert block" meta.doc;
          assert_bool ~name:"update metadata write command" true
            meta.write_command);

  check "Update.build_action rejects invalid uuid options" (fun () ->
      let base =
        {
          Update.id = None;
          uuid = Some "not-a-uuid";
          target_id = None;
          target_uuid = None;
          target_page = None;
          pos = None;
      ...
      in
      (match Update.build_action base (repo "logseq_db_alpha") with
      | Error err ->
          assert_equal ~name:"update invalid uuid code"
            (Edn_util.keyword_t "invalid-options")
            err.Error.code;
          assert_equal ~name:"update invalid uuid message"
            "Option uuid must be a valid UUID string" err.message
      | Ok _ -> failf "expected invalid source uuid");
      let target_opts =
        {
          base with
          id = Some 7L;
          uuid = None;
          target_uuid = Some "zzzzzzzz-zzzz-zzzz-zzzz-zzzzzzzzzzzz";
        }
      in
      match Update.build_action target_opts (repo "logseq_db_alpha") with
      | Error err ->
          assert_equal ~name:"update invalid target uuid code"
            (Edn_util.keyword_t "invalid-options")
            err.Error.code;
          assert_equal ~name:"update invalid target uuid message"
            "Option target-uuid must be a valid UUID string" err.message
      | Ok _ -> failf "expected invalid target uuid");

  check "Update.execute applies content and move operations" (fun () ->
      let config =
        test_config
          ~root_dir:(fresh_root "logseq-cli-update-execute-")
          ~base_url:"http://127.0.0.1:0" ~graph:"alpha" ()
      in
      with_http_server_sequence
        [
          (fun request ->
            assert_equal ~name:"update source pull line"
              "POST /v1/invoke HTTP/1.1" request.request_line;
            assert_contains ~name:"update source pull method"
              {|"method":"thread-api/pull"|} request.body;
            assert_contains ~name:"update source pull id" {|,7]|} request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
              {|{"resultTransit":"[\"^ \",\"~:db/id\",7,\"~:block/uuid\",\"~u11111111-1111-1111-1111-111111111111\",\"~:block/title\",\"Old\"]"}|}
            ));
          (fun request ->
            assert_equal ~name:"update target pull line"
              "POST /v1/invoke HTTP/1.1" request.request_line;
            assert_contains ~name:"update target pull method"
              {|"method":"thread-api/pull"|} request.body;
            assert_contains ~name:"update target page lookup" {|home|}
              request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
              {|{"resultTransit":"[\"^ \",\"~:db/id\",1,\"~:block/uuid\",\"~u22222222-2222-2222-2222-222222222222\",\"~:block/name\",\"home\",\"~:block/title\",\"Home\"]"}|}
            ));
          (fun request ->
            assert_equal ~name:"update apply line" "POST /v1/invoke HTTP/1.1"
              request.request_line;
            assert_contains ~name:"update apply method"
              {|"method":"thread-api/apply-outliner-ops"|} request.body;
            assert_contains ~name:"update save op" {|\"~:save-block\"|}
              request.body;
            assert_contains ~name:"update move op" {|\"~:move-blocks\"|}
              request.body;
            assert_contains ~name:"update source uuid"
              {|~u11111111-1111-1111-1111-111111111111|} request.body;
            assert_contains ~name:"update target uuid"
              {|~u22222222-2222-2222-2222-222222222222|} request.body;
            assert_contains ~name:"update content" {|Updated content|}
              request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
              {|{"resultTransit":"true"}|} ));
        ...
        (fun base_url ->
          ...
          let action =
            ...
            | Ok action -> action
            | Error err ->
                failf "unexpected update action error: %s" err.Error.message
          in
          let result =
            run_blocking (Update.execute action config Output.Mode.Json)
          in
          assert_bool ~name:"update execute ok" false
            (Cli_result.is_error result);
          match Cli_result.data_value result with
          | Some value when Option.is_some (Edn_util.as_map value) -> (
              match Edn_util.get value ":result" with
              | Some result when Edn_util.as_bool result = Some true -> ()
              | _ -> failf "expected true update result")
          | _ -> failf "expected structured update result"));

  check "Update.execute resolves named tags and properties before applying ops"
    (fun () ->
      let config =
        test_config
          ~root_dir:(fresh_root "logseq-cli-update-resolve-")
          ~base_url:"http://127.0.0.1:0" ~graph:"alpha" ()
      in
      with_http_server_sequence
        [
          (fun request ->
            assert_contains ~name:"update resolve source pull"
              {|"method":"thread-api/pull"|} request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
              {|{"resultTransit":"[\"^ \",\"~:db/id\",7,\"~:block/uuid\",\"~u11111111-1111-1111-1111-111111111111\",\"~:block/title\",\"Old\"]"}|}
          ...
          (fun request ->
            assert_contains ~name:"update resolve tag query"
              {|"method":"thread-api/q"|} request.body;
            assert_contains ~name:"update resolve tag name" {|project|}
              request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
              {|{"resultTransit":"[[\"^ \",\"~:db/id\",42,\"~:block/title\",\"project\"]]"}|}
            ));
          (fun request ->
            assert_contains ~name:"update resolve property pull"
              {|"method":"thread-api/pull"|} request.body;
            assert_contains ~name:"update resolve property ident" {|~:owner|}
              request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
              {|{"resultTransit":"[\"^ \",\"~:db/id\",9,\"~:db/ident\",\"~:owner\"]"}|}
            ));
          (fun request ->
            assert_contains ~name:"update resolve apply"
              {|"method":"thread-api/apply-outliner-ops"|} request.body;
            assert_contains ~name:"update resolve tag op"
              {|\"~:batch-set-property\"|} request.body;
            assert_contains ~name:"update resolve tag id" {|,42,|} request.body;
            assert_contains ~name:"update resolve property ident op"
              {|\"~:owner\"|} request.body;
            assert_contains ~name:"update resolve property value" {|Alice|}
              request.body;
            ( 200,
              [ ("Content-Type", "application/json") ],
              {|{"resultTransit":"true"}|} ));
        ...
        (fun base_url ->
          ...
          let action =
              ...
              Update.build_action
                {
                  ...
                  content = None;
                  update_tags_edn = Some {|["project"]|};
                  update_properties_edn = Some {|{"owner" "Alice"}|};
                  remove_tags_edn = None;
                  remove_properties_edn = None;
                  blocks_edn = None;
                  blocks_file = None;
            ...
            | Ok action -> action
            | Error err ->
                failf "unexpected update resolve action error: %s"
                  err.Error.message
          in
          let result =
            run_blocking (Update.execute action config Output.Mode.Json)
          in
          assert_bool ~name:"update resolve execute ok" false
            (Cli_result.is_error result)));

  check "Show.normalize_stdin_id accepts CLI id output forms" (fun () ->
      assert_opt_string ~name:"stdin blank id" None
        (Show.normalize_stdin_id (Some "   \n"));
      assert_opt_string ~name:"stdin single id" (Some "42")
        (Show.normalize_stdin_id (Some " 42\n"));
      assert_opt_string ~name:"stdin last vector line" (Some "[7,8]")
        (Show.normalize_stdin_id (Some "Count: 2\n[7,8]\n"));
      assert_opt_string ~name:"stdin whitespace ids" (Some "[7 8]")
        (Show.normalize_stdin_id (Some "7 8\n")));

  check "Show.resolve_target accepts normalized stdin id vectors" (fun () ->
      let opts =
        {
          Show.id_raw = None;
          uuid = None;
      ...
      in
      match Show.resolve_target opts with
      | Ok (Show.By_ids [ 7L; 8L ]) -> ()
      | Ok _ -> failf "expected show target to resolve stdin ids"
      | Error err -> failf "unexpected show target error: %s" err.Error.message);

  check "Db_worker_types.minimal_list_item keeps only CLI list fields"
    (fun () ->
      let raw =
        value_with
          [
            (":db/id", vint 1);
            (":block/title", vstr "Home");
            (":block/created-at", vint 10);
            (":block/updated-at", vint 20);
            (":db/ident", vkw ":logseq.class/Page");
            (":logseq.property/type", vkw ":page");
            (":extra", vstr "omitted");
          ]
      in
      let item = Db_worker_types.minimal_list_item (Entity.of_value raw) in
      let item_value = Edn_util.any item in
      assert_opt_int64 ~name:"minimal id" (Some 1L)
        (Edn_util.get_int64 item_value ":db/id");
      assert_opt_string ~name:"minimal title" (Some "Home")
        (Edn_util.get_string item_value ":block/title");
      match Edn_util.get item_value ":extra" with
      | None -> ()
      | Some _ -> failf "expected minimal item to omit extra fields");

  check "Db_worker_types.list_pages filters visible non-journal pages"
    (fun () ->
      let pages =
        vvec
          [
            value_with
              [
                (":db/id", vint 1);
                (":block/name", vstr "home");
                (":block/title", vstr "Home");
                (":block/created-at", vint 100);
                (":block/updated-at", vint 200);
              ];
            value_with
              [
                (":db/id", vint 2);
                (":block/name", vstr "hidden");
                (":block/title", vstr "Hidden");
                (":block/hidden?", vbool true);
              ];
            value_with
              [
                (":db/id", vint 3);
                (":block/name", vstr "builtin");
                (":block/title", vstr "Built In");
                (":logseq.property/built-in?", vbool true);
              ];
            value_with
              [
                (":db/id", vint 4);
                (":block/name", vstr "2024_01_01");
                (":block/title", vstr "2024-01-01");
                (":block/journal?", vbool true);
              ];
            value_with
              [
                (":db/id", vint 5);
                (":block/name", vstr "old");
                (":block/title", vstr "Old");
                (":block/created-at", vint 10);
                (":block/updated-at", vint 20);
              ];
          ]
      ...
      | [ page ] ->
          assert_opt_string ~name:"page label" (Some "Home") (Entity.label page);
          if page.Entity.kind <> Entity.Page then failf "expected page kind"
      | _ -> failf "expected only the visible non-journal page");

  check
    "Db_worker_types.list_tags and list_properties honor include-built-in \
     option" (fun () ->
      let items =
        vvec
          [
            value_with [ (":db/id", vint 1); (":block/title", vstr "Public") ];
            value_with
              [
                (":db/id", vint 2);
                (":block/title", vstr "Built In");
      ...
      in
      let options = value_with [ (":include-built-in", vbool false) ] in
      (match Db_worker_types.list_tags items options with
      | [ tag ] ->
          assert_opt_string ~name:"tag label" (Some "Public") (Entity.label tag);
          if tag.Entity.kind <> Entity.Tag then failf "expected tag kind"
      | _ -> failf "expected only the non-built-in tag");
      match Db_worker_types.list_properties items options with
      | [ property ] ->
          assert_opt_string ~name:"property label" (Some "Public")
            (Entity.label property);
          if property.Entity.kind <> Entity.Property then
            failf "expected property kind"
      | _ -> failf "expected only the non-built-in property");

  check "Db_worker_types.list_tasks filters by status priority and content"
    (fun () ->
      let tasks =
        vvec
          [
            value_with
              [
                (":db/id", vint 1);
                (":block/title", vstr "Write report");
                (":logseq.property/status", vkw ":logseq.property/status.doing");
                ( ":logseq.property/priority",
                  vkw ":logseq.property/priority.high" );
              ];
            value_with
              [
                (":db/id", vint 2);
                (":block/title", vstr "Write draft");
                (":logseq.property/status", vkw ":logseq.property/status.todo");
                ( ":logseq.property/priority",
                  vkw ":logseq.property/priority.high" );
              ];
            value_with
              [
                (":db/id", vint 3);
                (":block/title", vstr "Review report");
                (":logseq.property/status", vkw ":logseq.property/status.doing");
                ( ":logseq.property/priority",
                  vkw ":logseq.property/priority.low" );
              ];
          ]
      in
      let options =
        value_with
          [
            (":status", vkw ":logseq.property/status.doing");
            (":priority", vkw ":logseq.property/priority.high");
            (":content", vstr "report");
          ]
      in
      match Db_worker_types.list_tasks tasks options with
      | [ task ] ->
          assert_opt_string ~name:"task label" (Some "Write report")
            (Entity.label task);
          if task.Entity.kind <> Entity.Task then failf "expected task kind"
      | _ -> failf "expected only matching task");

  check "Db_worker_types.list_nodes filters by tag ids and property idents"
    (fun () ->
      let nodes =
        vvec
          [
            value_with
              [
                (":db/id", vint 1);
                (":block/title", vstr "Matched");
                ( ":block/tags",
                  vvec
                    [
                      value_with
                        [ (":db/id", vint 10); (":db/ident", vkw ":project") ];
                    ] );
                (":my/property", vstr "value");
              ];
            value_with
              [
                (":db/id", vint 2);
                (":block/title", vstr "Missing property");
                ( ":block/tags",
                  vvec
                    [
                      value_with
            ...
            value_with
              [
                (":db/id", vint 3);
                (":block/title", vstr "Missing tag");
                ( ":block/tags",
                  vvec
                    [
                      value_with
                        [ (":db/id", vint 11); (":db/ident", vkw ":other") ];
                    ] );
                (":my/property", vstr "value");
              ];
          ]
      ...
      let filter =
        {
          Db_worker_types.tag_ids = [ 10L ];
          property_idents = [ Edn_util.keyword_t ":my/property" ];
        }
      in
      match Db_worker_types.list_nodes nodes filter with
      | [ node ] ->
          assert_opt_string ~name:"node label" (Some "Matched")
            (Entity.label node);
          if node.Entity.kind <> Entity.Page then
            failf "expected page node kind"
      | _ -> failf "expected only matching node");
  ...
  check "Agent.resolve_agent_name follows cli config and hostname" (fun () ->
      let root = fresh_root "logseq-cli-agent-name-" in
      let config = test_config ~root_dir:root () in
      assert_equal ~name:"agent hostname fallback" "build-host"
        (Result.get_ok (Agent.resolve_agent_name config (Some "build-host")));
      let config_with_agent =
        {
          config with
          raw_file_config =
            Some
              (Edn_util.map
                 [
                   (Edn_util.keyword ":agent-name", Edn_util.string " Worker A ");
                 ]);
        }
      in
      assert_equal ~name:"agent configured trim" "Worker A"
        (Result.get_ok
           (Agent.resolve_agent_name config_with_agent (Some "build-host")));
      let config_with_blank_agent =
        {
          config with
          raw_file_config =
            Some
      ...
      in
      match
        Agent.resolve_agent_name config_with_blank_agent (Some "build-host")
      with
      | Error err ->
          assert_equal ~name:"agent blank code"
            (Edn_util.keyword_t "agent-name-invalid")
            err.Error.code;
          assert_equal ~name:"agent blank message"
            "agent-name in cli.edn must be a non-empty string" err.message
      | Ok _ -> failf "expected configured blank agent name to fail");

  check "Agent.routable_task_decision matches AgentBridge task filters"
    (fun () ->
      let task_entity fields =
        Entity.of_value
          (Edn_util.map
             (List.map
                (fun (key, value) -> (Edn_util.keyword key, value))
                fields))
      in
      let task_tag =
        Edn_util.map
          [
            (Edn_util.keyword ":db/ident", Edn_util.keyword ":logseq.class/Task");
          ]
      in
      let status_todo =
        Edn_util.map
      ...
      let alice =
        Edn_util.map
          [ (Edn_util.keyword ":block/title", Edn_util.string "agent-a") ]
      in
      let base =
        [
          (":block/uuid", Edn_util.uuid "11111111-1111-1111-1111-111111111111");
          (":block/tags", Edn_util.vector [ task_tag ]);
          (":logseq.property/status", status_todo);
          (":logseq.property/assignee", Edn_util.vector [ alice ]);
        ]
      in
      assert_bool ~name:"agent routable" true
        (Agent.routable_task_decision (task_entity base) ~agent_name:"agent-a"
        = Agent.Routable);
      assert_bool ~name:"agent missing uuid" true
        (Agent.routable_task_decision
           (task_entity (List.remove_assoc ":block/uuid" base))
           ~agent_name:"agent-a"
        = Agent.Not_routable Agent.Missing_stable_uuid);
      assert_bool ~name:"agent assignee mismatch" true
        (Agent.routable_task_decision (task_entity base) ~agent_name:"agent-b"
        = Agent.Not_routable Agent.Assignee_mismatch);
      assert_bool ~name:"agent already routed" true
        (Agent.routable_task_decision
           (task_entity
              (base
              @ [
                  ( ":logseq.property.agent/session-id",
                    Edn_util.string "session-1" );
                ]))
           ~agent_name:"agent-a"
        = Agent.Not_routable Agent.Already_routed));

  check "Agent prompt templates validate and render variables" (fun () ->
      let template =
        {
          ...
          body =
            "Graph {{graph}}\n\
             ...
             {{task-block-tree}}";
          required_vars =
            [ "graph"; "block-uuid"; "agent-name"; "task-block-tree" ];
          allowed_vars =
            [ "graph"; "block-uuid"; "agent-name"; "task-block-tree" ];
        }
      in
      (match Agent.validate_prompt_template template with
      | Ok () -> ()
      | Error err -> failf "unexpected template error: %s" err.Error.message);
      assert_equal ~name:"agent prompt render"
        "Graph alpha\nBlock uuid-1\nAgent agent-a\nTask tree"
        (Result.get_ok
           (Agent.render_prompt_template template
              [
                ("graph", "alpha");
                ("block-uuid", "uuid-1");
                ("agent-name", "agent-a");
      ...
      let unknown = { template with body = template.body ^ "\n{{unknown}}" } in
      match Agent.validate_prompt_template unknown with
      | Error err ->
          assert_equal ~name:"agent unknown template code"
...
|});
  ...
  check "Server_runtime.list_servers discovers server-list health entries"
    (fun () ->
      let root = fresh_root "logseq-cli-server-list-" in
      Unix.mkdir root 0o755;
      with_http_server_port
        (fun port _request ->
          ( 200,
            [ ("Content-Type", "application/json") ],
            {|{"repo":"alpha","status":"ready","host":"127.0.0.1","port":|}
            ^ string_of_int port
        ...
        (fun base_url ->
          ...
          let config = test_config ~root_dir:root () in
          let servers = run_blocking (Server_runtime.list_servers config) in
          match servers with
          | [ server ] ->
              assert_equal ~name:"server repo" "logseq_db_alpha"
                (repo_string server.Server_runtime.repo);
              assert_equal ~name:"server base url" base_url server.base_url;
              if server.status <> Server_runtime.Ready then
                failf "expected ready server";
              if not server.owned then failf "expected cli-owned server"
          | _ -> failf "expected one discovered server"));

  check "Server_runtime.ensure_server uses discovered server-list entry"
    (fun () ->
      let root = fresh_root "logseq-cli-ensure-server-" in
      Unix.mkdir root 0o755;
      with_http_server_port
        (fun port _request ->
          ( 200,
        ...
        (fun base_url ->
          ...
          | Error err ->
              failf "unexpected ensure_server error: %s" err.Error.message));

  check "Server_runtime.list_graphs returns canonical graph names" (fun () ->
      let root = fresh_root "logseq-cli-server-runtime-graphs-" in
      ensure_test_dir root;
      ensure_test_dir (Filename.concat root "graphs");
      ensure_test_dir (Filename.concat root "graphs/beta");
      ensure_test_dir (Filename.concat root "graphs/alpha");
      ensure_test_dir (Filename.concat root "graphs/foo~2Fbar");
      ensure_test_dir (Filename.concat root "graphs/logseq_db_alpha");
      ensure_test_dir (Filename.concat root "graphs/Unlinked graphs");
      write_text_file (Filename.concat root "graphs/not-a-dir") "";
      let config = test_config ~root_dir:root () in
      assert_equal ~name:"server runtime graphs" "alpha,beta,foo/bar"
        (String.concat ","
           (List.map graph_string (Server_runtime.list_graphs config))));

  check "Server_runtime.list_graph_items returns shared graph item records"
    (fun () ->
      let root = fresh_root "logseq-cli-server-runtime-graph-items-" in
      ensure_test_dir root;
      ensure_test_dir (Filename.concat root "graphs");
      ensure_test_dir (Filename.concat root "graphs/alpha");
      ensure_test_dir (Filename.concat root "graphs/foo++bar");
      ...
      ensure_test_dir (Filename.concat root "graphs/logseq_db_alpha");
      let config = test_config ~root_dir:root () in
      let items = Server_runtime.list_graph_items config in
      assert_int ~name:"graph item count" 3 (List.length items);
      let find_by_legacy legacy_dir =
        List.find_opt
          (fun item -> item.Graph_types.legacy_dir = Some legacy_dir)
          items
      in
      (match List.hd items with
      | { Graph_types.kind = Canonical; graph_name = Some graph_name; _ }
        when graph_string graph_name = "alpha" ->
          ()
      | _ -> failf "expected alpha canonical item first");
      (match find_by_legacy "foo++bar" with
      | Some
          {
            Graph_types.kind = Legacy;
            graph_name = Some graph_name;
            target_graph_dir = Some "foo~2Fbar";
            conflict = false;
            _;
          }
        when graph_string graph_name = "foo/bar" ->
          ()
      | _ -> failf "expected legacy foo/bar graph item");
      match find_by_legacy "bad%ZZname" with
      | Some { Graph_types.kind = Legacy_undecodable; reason = Some reason; _ }
        when reason = Edn_util.keyword_t ":graph-name-not-derivable" ->
          ()
      | _ -> failf "expected undecodable legacy graph item");

  check "Server_runtime uses encoded canonical repo dirs" (fun () ->
      let root = fresh_root "logseq-cli-server-runtime-encoded-" in
      ensure_test_dir root;
      ...
      | Error err ->
          assert_equal ~name:"encoded repo dir start error"
            (Edn_util.keyword_t "server-script-missing")
            err.Error.code;
          assert_bool ~name:"encoded repo dir exists" true
            (Sys.file_exists (Filename.concat root "graphs/foo~2Fbar"));
          assert_bool ~name:"raw nested repo dir absent" false
            (Sys.file_exists (Filename.concat root "graphs/foo/bar")));
