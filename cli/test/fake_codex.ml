...
let argv_contains value =
  Array.exists (( = ) value) Node.Process.argv

let () =
  if argv_contains "--version" then
    Node.Process.exit 0;
  let marker =
  ...
  in
  let worker_script =
    match Js.Dict.get Node.Process.process##env "LOGSEQ_DB_WORKER_NODE_SCRIPT" with
    | Some value -> value
    | None -> ""
  in
  print_endline "{\"session_id\":\"cli-live-session\"}";
  set_timeout (fun [@u] () -> print_endline "still alive after session id") 200;
  set_timeout (fun [@u] () -> Node.Fs.writeFileAsUtf8Sync marker worker_script) 500;
