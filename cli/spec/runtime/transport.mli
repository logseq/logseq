...
val write_output :
  ...
  data:Melange_edn.any ->
  unit Error.build_result Cli_effect.t

val read_input :
  format:Cli_primitive.keyword ->
  path:Cli_primitive.path ->
  Melange_edn.any Error.build_result Cli_effect.t
...
val thread_api_db_sync_list_remote_graphs :
  invoke_config -> Melange_edn.any Cli_effect.t
...
val thread_api_db_sync_start :
  invoke_config -> repo:Cli_primitive.repo -> Melange_edn.any Cli_effect.t

val thread_api_db_sync_status :
  invoke_config -> repo:Cli_primitive.repo -> Melange_edn.any Cli_effect.t

val thread_api_db_sync_stop : invoke_config -> Melange_edn.any Cli_effect.t
...
val thread_api_get_e2ee_password :
  invoke_config -> refresh_token:string -> Melange_edn.any Cli_effect.t
...
val thread_api_pull :
  invoke_config ->
  repo:Cli_primitive.repo ->
  selector:Melange_edn.vector Melange_edn.t ->
  lookup:Melange_edn.any ->
  Melange_edn.any Cli_effect.t
...
val thread_api_set_db_sync_config :
  invoke_config ->
  config:Melange_edn.map Melange_edn.t ->
  Melange_edn.any Cli_effect.t
