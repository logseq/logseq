...
let invalid_options = function
  | Parsed_block opts
    when count_some (Vec.of_array [| opts.id_raw; opts.uuid |]) > 1 ->
      Some "only one of --id or --uuid is allowed"
  | Parsed_page opts
    when count_some
           (Vec.of_array [| Option.map Int64.to_string opts.id; opts.page |])
         > 1 ->
  ...
  | (Parsed_tag opts | Parsed_property opts)
    when count_some
           (Vec.of_array [| Option.map Int64.to_string opts.id; opts.name |])
         > 1 ->
...
let build_block repo graph (opts : block_opts) =
  ...
  | None, None -> Error (Error.make Error.Missing_target "block is required")
  | Some id_raw, None -> (
...
let build_named make repo graph (opts : named_entity_opts) =
  ...
  | None, None ->
      Error (Error.make Error.Missing_target "name or id is required")
...
let block_selector =
  vector_vec (Vec.of_array [| kw "db/id"; kw "block/uuid"; kw "block/name" |])
  ...
  let fields =
    Vec.of_array
      [|
        ( kw "deleted-ids",
          Edn_util.vector_vec
            (deleted_ids |> Vec.map (fun id -> Edn_util.int64 id)) );
        ( kw "missing-ids",
          Edn_util.vector_vec
            (missing_ids |> Vec.map (fun id -> Edn_util.int64 id)) );
        (kw "result", result);
      |]
  in
  let fields =
    if Vec.is_empty page_ids then fields
    else
      let ids = page_ids in
         ...
         [|
           ...
           Edn_util.vector_vec
             (Vec.of_array
                [|
                  Edn_util.vector_vec
                    (uuids |> Vec.map (fun uuid -> Edn_util.uuid uuid));
...
let ambiguous_error code label name matches =
  Error.make
    ~candidates:(Vec.filter_map candidate_of_entity matches)
    code
...
let validate_tag_target entity =
  if not (has_id entity) then
    ...
    Error (Error.make Error.Invalid_tag_target "target is not a tag")
  else if Edn_util.get_bool entity "logseq.property/built-in?" = Some true then
    Error (Error.make Error.Tag_built_in "built-in tag cannot be removed")
  else if Edn_util.get_bool entity "logseq.property/public?" = Some false then
    Error (Error.make Error.Tag_hidden "hidden tag cannot be removed")
  else
    match uuid_of_entity entity with
    | Some _ -> Ok entity
    | None -> Error (Error.make Error.Tag_not_found "tag uuid not found")

let validate_property_target entity =
  if not (has_id entity) then
    Error (Error.make Error.Property_not_found "property not found")
  else if not (property_entity entity) then
    Error (Error.make Error.Invalid_property_target "target is not a property")
  else if Edn_util.get_bool entity "logseq.property/built-in?" = Some true then
    Error
      (Error.make Error.Property_built_in "built-in property cannot be removed")
  else if Edn_util.get_bool entity "logseq.property/public?" = Some false then
    Error (Error.make Error.Property_hidden "hidden property cannot be removed")
  else
    match uuid_of_entity entity with
    | Some _ -> Ok entity
    | None ->
        Error (Error.make Error.Property_not_found "property uuid not found")
...
let delete_resolved_block mode invoke_config repo entity =
  ...
  if not (has_id entity) then
    ...
    pure
      (Cli_result.error ~command:Command_id.Remove_block mode
         (Error.make Error.Invalid_target
    ...
    | None ->
        pure
          (Cli_result.error ~command:Command_id.Remove_block mode
             (Error.make Error.Block_not_found "block uuid not found"))
    | Some uuid ->
        bind
          (delete_block_uuids invoke_config repo (Vec.singleton uuid))
          (fun result ->
            pure
...
let execute_remove_block_uuid mode invoke_config repo uuid =
  ...
  bind (pull invoke_config repo block_selector lookup) (fun entity ->
      if has_id entity then delete_resolved_block mode invoke_config repo entity
        ...
        let string_lookup =
          vector_vec (Vec.of_array [| kw "block/uuid"; Edn_util.string uuid |])
        in
        bind
...
let execute_remove_block_ids mode invoke_config repo ids =
  ...
  bind entity_effects (fun entities ->
      let id_entities = Vec.combine ids entities in
      let deleted_ids =
        id_entities
        |> Vec.filter_map (fun (id, entity) ->
            if has_id entity && not (has_name entity) then Some id else None)
      in
      let deleted_uuids =
        id_entities
        |> Vec.filter_map (fun (_, entity) ->
      ...
      in
      if Vec.is_empty deleted_ids && not (Vec.is_empty page_ids) then
        pure
          (Cli_result.error ~command:Command_id.Remove_block mode
             (Error.make Error.Invalid_target
                "target is not a block, use 'remove page' instead"))
        ...
        let delete_effect =
          if Vec.is_empty deleted_uuids then pure Edn_util.nil
...
let execute_remove_entity mode invoke_config repo ~command ~list_method
  ...
  | None, Some name ->
      bind (list_named_entities invoke_config repo list_method)
        (fun items_value ->
          let matches =
            items_value |> entities_of_value |> Vec.filter (name_matches name)
          in
          if Vec.is_empty matches then
            pure
              (Cli_result.error ~command mode
            ...
            let lookup =
              vector_vec
                (Vec.of_array
                   [| kw "block/name"; Edn_util.string lookup_name |])
            in
            let resolved_name = entity_name item in
            bind (pull invoke_config repo entity_selector lookup) (fun entity ->
                delete entity resolved_name))
  | None, None ->
      pure
        (Cli_result.error ~command mode
           (Error.make Error.Missing_target (label ^ " name or id is required")))
...
let execute_with_mode action config mode =
  ...
  | Remove_block { repo; ids; multi_id = true; _ } when not (Vec.is_empty ids)
    ->
      bind (Server_runtime.ensure_server config repo ~create_empty_db:false)
        (function
        | Error err ->
  ...
  | Remove_page _ ->
      pure
        (Cli_result.error ~command:Command_id.Remove_page mode
           (Error.make Error.Missing_page_name "page name or id is required"))
      ...
      bind (Server_runtime.ensure_server config repo ~create_empty_db:false)
        ...
        | Ok invoke_config ->
            execute_remove_entity mode invoke_config repo
              ...
              ~ambiguous_code:Error.Ambiguous_tag_name ~label:"tag"
              ~validate:validate_tag_target ~id ~name)
      ...
      bind (Server_runtime.ensure_server config repo ~create_empty_db:false)
        ...
        | Ok invoke_config ->
            execute_remove_entity mode invoke_config repo
              ...
              ~ambiguous_code:Error.Ambiguous_property_name ~label:"property"
              ~validate:validate_property_target ~id ~name)
