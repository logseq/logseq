...
(defn- imported-entities-missing-tx-id
  [db]
  (d/q '[:find [?e ...]
         :where
         [?e :block/uuid]
...
(defn ^:api finalize-imported-graph!
  ...
  transact-pipeline skip refs. Desktop then stamps tx-id without ::new-graph?
  and the pipeline rebuilds refs. CLI has no worker pipeline, so this pass
  writes both in one transact."
  [conn]
  (let [db @conn
        entity-ids (imported-entities-missing-tx-id db)]
    (when (seq entity-ids)
            ...
            blocks (keep (fn [id]
                           (let [block (d/entity db id)]
                             (when (and block
                               ...
                               block)))
                         entity-ids)
            refs-tx (vec (rebuild-block-refs-tx {:db-after db} blocks))]
        (ldb/transact! conn
