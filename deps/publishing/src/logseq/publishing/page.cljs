(ns logseq.publishing.page
  "Renders published index.html. Kept free of datascript so nbb tests can cover
  hosted script/src paths without loading the graph DB."
...
(defn ^:large-vars/html index-html
  "Returns the published index.html string for the given transit db and app state."
      ...
      (function(l) {
        if (l.search) {
          ...
          if (q.p !== undefined) {
            ...
            ;; Deferring main.js results in errors. React is bundled into main.js.
            (map (fn [attrs]
                   [:script attrs])
                 publish-runtime/page-js-scripts)))))))
