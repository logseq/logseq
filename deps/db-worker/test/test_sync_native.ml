...
let () =
  ...
  with_client_ops_db repo (fun db ->
      ...
      upsert ~tx_id:keep_tx_id ~created_at:(Time.epoch_ms 1L) ~pending:false;
      upsert ~tx_id:remove_tx_id ~created_at:(Time.epoch_ms 2L) ~pending:false;
      upsert ~tx_id:pending_tx_id ~created_at:(Time.epoch_ms 3L) ~pending:true;
