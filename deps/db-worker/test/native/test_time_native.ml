(* Tests for the Time platform spec (spec/platform/time.mli), native
   target backed by Unix.localtime/mktime + pure fixed-offset
   arithmetic.

   Mirrors test/test_time_melange.ml — same test names in the same
...
let int64_eq actual expected = Alcotest.(check int64) "int64" expected actual
let int_eq actual expected = Alcotest.(check int) "int" expected actual
let string_eq actual expected =
  Alcotest.(check string) "string" expected actual

let civil_fields_testable =
  Alcotest.testable
    (fun fmt (y, mo, d, h, mi, s, ms) ->
      Format.fprintf fmt "%d-%02d-%02d %02d:%02d:%02d.%03d" y mo d h mi s ms)
    ( = )

let fields_eq actual expected =
  Alcotest.(check civil_fields_testable) "civil_fields" expected
    (Time.civil_fields actual)

let fields_eq3 actual expected = ok (actual = expected)
...
let test_epoch_civil_roundtrip () =
  List.iter
...
let test_epoch_ms_of_civil_rollover () =
...
let test_local_date () =
  (match Time.local_date ~year:2026 ~month:3 ~day:15 ~tz:Time.utc with
   | Some d -> fields_eq3 (Time.local_date_fields d) (2026, 3, 15)
   | None -> ok false);
  (match Time.local_date ~year:2024 ~month:2 ~day:29 ~tz:Time.utc with
   | Some d -> fields_eq3 (Time.local_date_fields d) (2024, 2, 29)
   | None -> ok false);
  ...
  ok (Time.local_date ~year:2023 ~month:2 ~day:29 ~tz:Time.utc = None);
  let utc_d =
...
let test_journal_day () =
  ...
  (match Time.local_date_of_journal_day Time.utc 20260315 with
   | Some d ->
       fields_eq3 (Time.local_date_fields d) (2026, 3, 15);
...
let test_iso_string_of_epoch_ms () =
  string_eq
...
let test_epoch_ms_of_iso_string () =
...
let () =
  Alcotest.run "time"
    [ ( "time"
      , [ Alcotest.test_case "epoch-ms-basics-test" `Quick
            test_epoch_ms_basics
        ; Alcotest.test_case "epoch-civil-roundtrip-test" `Quick
            test_epoch_civil_roundtrip
        ; Alcotest.test_case "epoch-ms-of-civil-rollover-test" `Quick
            test_epoch_ms_of_civil_rollover
        ; Alcotest.test_case "local-date-test" `Quick test_local_date
        ; Alcotest.test_case "journal-day-test" `Quick test_journal_day
        ; Alcotest.test_case "iso-string-of-epoch-ms-test" `Quick
            test_iso_string_of_epoch_ms
        ; Alcotest.test_case "epoch-ms-of-iso-string-test" `Quick
            test_epoch_ms_of_iso_string
        ; Alcotest.test_case "monotonic-test" `Quick test_monotonic
        ; Alcotest.test_case "local-tz-test" `Quick test_local_tz ] )
    ]
