...
(* :thread-api/export-blocks-as-format [repo root-block-uuids opts format]
   cljs resolves the format through the npm mldoc lib (gp-mldoc/->db-edn)
   for :markdown and through gp-block/* for :edn/:json. mldoc is JS-only,
   so this endpoint fails fast rather than silently producing wrong output. *)
let export_blocks_as_format args =
  let _repo = repo_of args in
  invalid_arg
    "export-blocks-as-format is unavailable: mldoc (gp-mldoc/->db-edn) has \
     no OCaml port"
