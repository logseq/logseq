...
let export_doc_files (conn : conn) (raw_doc_files : BM.t list)
  ...
  (seq_eff
   ...
   seq_eff
     ...
     other_files)
  >>= fun () ->
  Gp_exporter_finalize.set_finishing_import_ui options.set_ui_state;
  import_progress options [ ("phase", kw "normalize-journal-uuids") ];
  (match Gp_exporter_finalize.normalize_journal_uuids conn with
   | Some r -> options.on_tx_report r
   | None -> ());
  import_progress options [ ("phase", kw "cleanup-missing-block-refs") ];
  let cleanup_report =
    Gp_exporter_finalize.cleanup_missing_block_refs conn
  ...
  (match cleanup_report with
   | Some r -> options.on_tx_report r
   | None -> ());
  (match Gp_exporter_finalize.ensure_imported_page_parent_orders conn with
   | Some r -> options.on_tx_report r
   | None -> ());
  (if options.finalize_imported_graph then (
     ...
     in
     ignore (Gp_exporter_finalize.finalize_imported_graph conn);
