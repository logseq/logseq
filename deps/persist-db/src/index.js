...
const SQLiteDB = {
    ...
    async rtcInit(dbName) {
        let db = dbMap[dbName];
        if (!db) {
            await this.newDB(dbName);
            db = dbMap[dbName];
        }
        const sql = `CREATE TABLE IF NOT EXISTS rtc_ops (
        ...
        sqlite3.str_finish(str);
        sqlite3.finalize(prepared.stmt);
        console.log("rtc init", rc);
        return;
    },

    async rtcAddOps(dbName, rawOps) {
        let db = dbMap[dbName];
        if (!db) {
            await this.newDB(dbName);
            db = dbMap[dbName];
        }
        const sql = "INSERT INTO rtc_ops (v) VALUES (@v)";
        const str = sqlite3.str_new(db, sql);
        const prepared = await sqlite3.prepare_v2(db, sqlite3.str_value(str));
        sqlite3.bind(prepared.stmt, 1, rawOps);
        const rc2 = await sqlite3.step(prepared.stmt);
        console.log("rtc add ops step", rc2);
        sqlite3.str_finish(str);
        sqlite3.finalize(prepared.stmt);
        return;
    },

    async rtcGetOps(dbName) {
        let db = dbMap[dbName];
        if (!db) {
            await this.newDB(dbName);
            db = dbMap[dbName];
        }
        const sql = "SELECT * FROM rtc_ops";
        const str = sqlite3.str_new(db, sql);
        const prepared = await sqlite3.prepare_v2(db, sqlite3.str_value(str));
        const ops = [];
        while (await sqlite3.step(prepared.stmt) === SQLite.SQLITE_ROW) {
        ...
        sqlite3.str_finish(str);
        sqlite3.finalize(prepared.stmt);
        console.log("rtc get ops", ops);
        return ops;
    },

    async rtcClearOps(dbName) {
        let db = dbMap[dbName];
        if (!db) {
            await this.newDB(dbName);
            db = dbMap[dbName];
        }
        const sql = "DELETE FROM rtc_ops WHERE id <> 0";
        const str = sqlite3.str_new(db, sql);
        const prepared = await sqlite3.prepare_v2(db, sqlite3.str_value(str));
        const rc = await sqlite3.step(prepared.stmt);
        sqlite3.str_finish(str);
        sqlite3.finalize(prepared.stmt);
        console.log("rtc clear ops", rc);
        return;
    }
