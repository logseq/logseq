...
(defn markdown-heading-level
  "Heading level 1-6 when `content` starts with markdown heading markers."
  [content]
  (when (string? content)
    (when-let [heading (re-find #"^(#{1,6})\s+" (string/triml content))]
      (count (second heading)))))

(defn heading-value->level
  "Numeric heading level from a DB heading property. `true` is auto-heading."
  [heading level]
  (cond
