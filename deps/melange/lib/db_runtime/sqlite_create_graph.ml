...
let keyword_text runtime value =
  ...
  then invalid_arg "Graph construction expects a keyword";
...
let qualifyPropertySchemaWith runtime property_schema schema_properties_map =
  ...
  |> Array.map (function
    | [| key; value |] ->
        let key =
          if
            Melange_cljs_runtime_spec.Value_codec.value_is_keyword runtime key
            && not
                 (String.contains
...
let build_properties_with build_property runtime
    ...
    (generate_order : Property_build.value_callback) (now_ms : float_callback)
    built_in_properties property_catalog schema_properties_map =
  ...
  let build_one = function
    | [| catalog_ident; definition |] ->
        ...
        let db_ident =
          if
            Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime attribute
          then catalog_ident
          else attribute
        in
        let schema =
              ...
              | [| property_ident; value |]
                when not (internal_ident runtime value) ->
                  let property_definition =
                    Melange_cljs_runtime_spec.Value_codec.map_get runtime
                      property_catalog property_ident
                  in
                  if
                    Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime
                      property_definition
        ...
        let property_value_transactions =
          Property_build.build_property_value_entries_with runtime generate_uuid
            generate_order now_ms property property_value_entries
...
type encoded_initial_options = {
  importType : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t;
  graphGitSha : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t;
...
let select_keys runtime map names =
  Rrbvec.fold_left
    (fun result name ->
      ...
      in
      if Melange_cljs_runtime_spec.Value_codec.map_contains runtime map key then
...
let hidden_page runtime (now_ms : float_callback) ~title ~name =
  ...
  |> assoc runtime "block/uuid"
       ...
       |> Melange_cljs_runtime_spec.Value_codec.uuid_from_string runtime)
  |> assoc runtime "block/name"
       (Melange_cljs_runtime_spec.Value_codec.string_to_value runtime name)
  |> assoc runtime "block/title"
       (Melange_cljs_runtime_spec.Value_codec.string_to_value runtime title)
...
let build_initial_data_with build_property build_properties runtime
    (generate_uuid : Property_build.value_callback)
    (generate_order : Property_build.value_callback) (now_ms : float_callback)
    (now_instant : Property_build.value_callback) config_content
    built_in_properties schema_properties_map built_in_classes schema_version
    (options : encoded_initial_options) =
  let keyword =
    Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime
  in
  ...
  let build_bootstrap_property ident =
    ...
    let definition =
      Melange_cljs_runtime_spec.Value_codec.map_get runtime built_in_properties
        db_ident
    in
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime definition
    then invalid_arg ("Missing bootstrap property " ^ ident);
    let schema =
      qualifyPropertySchemaWith runtime
        (field runtime definition "schema")
        schema_properties_map
  ...
  let remaining_properties =
    Rrbvec.fold_left
      (fun properties ident ->
        Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime properties
          (keyword ident))
  ...
  let db_ident_to_properties =
    ...
    |> Rrbvec.filter_map (fun property ->
        ...
        if not is_property then None
          ...
          if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime ident
          then None
  ...
  let classes_tx =
    let without_idents =
      Rrbvec.map
        (fun class_ ->
          Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime class_
            (keyword "db/ident"))
        bootstrap_classes
    in
    let remaining =
    ...
    Rrbvec.of_array
      [|
        kv "logseq.kv/db-type"
          (Melange_cljs_runtime_spec.Value_codec.string_to_value runtime "db");
  ...
  let initial_data =
    ...
    | Some import_type
      when Melange_cljs_runtime_spec.Value_codec.value_truthy runtime
             import_type ->
  ...
  let initial_data =
    ...
    | Some sha
      when Melange_cljs_runtime_spec.Value_codec.value_truthy runtime sha ->
  ...
  let generated_uuid =
    Melange_cljs_runtime_spec.Value_codec.uuid_to_string runtime generated_uuid
  in
  ...
  let make_file path content =
    ...
    |> assoc runtime "file/path"
         (Melange_cljs_runtime_spec.Value_codec.string_to_value runtime path)
    |> assoc runtime "file/content"
         (Melange_cljs_runtime_spec.Value_codec.string_to_value runtime content)
