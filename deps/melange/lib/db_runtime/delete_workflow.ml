...
let optional_field runtime datascript value name =
  ...
  if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime field then None
  else Some field
...
let optional_uuid runtime datascript value name =
  optional_field runtime datascript value name
  |> Option.map (Melange_cljs_runtime_spec.Value_codec.uuid_to_string runtime)
...
let snapshot runtime datascript value :
    Melange_cljs_runtime_spec.Value_codec.cljs_value Domain.snapshot =
  let histories =
    Rrbvec.empty
    |> Rrbvec.append
...
let decode_transaction runtime source :
    Melange_cljs_runtime_spec.Value_codec.cljs_value Domain.transaction =
  let kind =
    if Melange_cljs_runtime_spec.Value_codec.value_is_map runtime source then
      Domain.Map_tx source
    else if
      not (Melange_cljs_runtime_spec.Value_codec.value_is_vector runtime source)
    then Other_tx
    else
      let values =
        Melange_cljs_runtime_spec.Value_codec.vector_to_array runtime source
      in
      let get index =
...
let capabilities runtime datascript :
    ...
    Domain.capabilities =
  {
    ...
    integer =
      (fun value ->
        if Melange_cljs_runtime_spec.Value_codec.value_is_integer runtime value
        then
          Some
            (Melange_cljs_runtime_spec.Value_codec.int_from_value runtime value)
        else None);
    int_value = Melange_cljs_runtime_spec.Value_codec.int_to_value runtime;
    uuid_text =
      (fun value ->
        if Melange_cljs_runtime_spec.Value_codec.value_is_uuid runtime value
        then
          Some
            (Melange_cljs_runtime_spec.Value_codec.uuid_to_string runtime value)
        else if
      ...
      vector runtime
        (Rrbvec.of_array
           [|
             ...
             Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime
               "block/refs";
             Melange_cljs_runtime_spec.Value_codec.int_to_value runtime block_id;
           |])
