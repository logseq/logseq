...
type parent_error_callback =
  (Melange_cljs_runtime_spec.Value_codec.cljs_value -> exn[@u])
...
type append_collector_callback =
  (collector -> Melange_datascript_spec.Api.datom array -> unit[@u])

type collector_value_callback =
  (collector -> Melange_cljs_runtime_spec.Value_codec.cljs_value[@u])

type clear_collector_callback = (collector -> unit[@u])

type temp_batch_callback =
  (Melange_datascript_spec.Api.connection -> collector -> unit[@u])

type batch_callback = (Melange_datascript_spec.Api.connection -> unit[@u])

type listener_callback =
  (Melange_datascript_spec.Api.transaction_report -> unit[@u])

type before_commit_callback = (unit -> unit[@u])

type commit_callback =
  (Melange_datascript_spec.Api.connection ->
   Melange_datascript_spec.Api.transaction_data ->
   Melange_datascript_spec.Api.transaction_metadata ->
   Melange_cljs_runtime_spec.Value_codec.cljs_value
  [@u])

type nested_error_callback =
  (Melange_cljs_runtime_spec.Value_codec.cljs_value -> exn[@u])
...
type local_connection_callback =
  (Melange_cljs_runtime_spec.Value_codec.cljs_value ->
   Melange_datascript_spec.Api.connection Js.Nullable.t
  [@u])
...
let metadata_flag runtime metadata name =
  Melange_cljs_runtime_spec.Value_codec.map_get runtime metadata
...
let valid_page_parent runtime datascript report =
  let database =
    Melange_datascript_spec.Api.report_db_after datascript report
  in
  let parent_attribute =
    Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime
      "block/parent"
  in
  Melange_datascript_spec.Api.report_datoms datascript report
  |> Array.exists (fun datom ->
      Melange_datascript_spec.Api.datom_added datascript datom
...
let batchWithTemp runtime datascript execution connection metadata
  ...
  let capabilities =
    ({
       database = Melange_datascript_spec.Api.database datascript;
       create_temporary =
         Melange_datascript_spec.Api.create_temporary_connection datascript;
       create_collector = (fun () -> (create_collector () [@u]));
       listen =
         (fun connection callback ->
           Melange_datascript_spec.Api.listen datascript connection listener_key
             (fun[@u] report -> callback report));
       unlisten =
         (fun connection ->
           Melange_datascript_spec.Api.unlisten datascript connection
             listener_key);
       append_report =
         (fun collector report ->
           (append_collector collector
              (Melange_datascript_spec.Api.report_datoms datascript report) [@u]));
       notify_listener =
         (fun report ->
       ...
       release_connection =
         Melange_datascript_spec.Api.release_connection datascript;
...
let batchWith runtime datascript execution connection metadata batch_callback
  ...
  let capabilities =
    ({
       database = Melange_datascript_spec.Api.database datascript;
       batch_active =
         Melange_datascript_spec.Api.connection_batch_active datascript;
       ...
       notify_listener =
         (fun report ->
           Option.iter (fun callback -> (callback report [@u])) listener);
       begin_batch = Melange_datascript_spec.Api.begin_batch datascript;
       invoke_batch = (fun connection -> (batch_callback connection [@u]));
       end_batch = Melange_datascript_spec.Api.end_batch datascript;
       storage_exists =
         (fun database ->
           Melange_datascript_spec.Api.storage_for datascript database
           |> Js.Nullable.toOption |> Option.is_some);
       store = Melange_datascript_spec.Api.store datascript;
       mark_database_stored =
         Melange_datascript_spec.Api.mark_database_stored datascript;
       final_metadata =
         (fun metadata ->
           Melange_cljs_runtime_spec.Value_codec.map_assoc runtime metadata
             (Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime
       ...
       make_report =
         (fun database_before database_after metadata datoms ->
           Melange_datascript_spec.Api.make_transaction_report datascript
             database_before database_after metadata (Rrbvec.to_array datoms));
       run_callbacks = Melange_datascript_spec.Api.run_callbacks datascript;
       log_error = (fun error -> (log_batch_error error [@u]));
       reset_database = Melange_datascript_spec.Api.reset_database datascript;
     }
      : ( Melange_datascript_spec.Api.connection,
          Melange_datascript_spec.Api.database,
          Melange_datascript_spec.Api.transaction_metadata,
...
let transaction_validation_options : Validation_schema.encoded_workflow_options
    =
  {
  ...
  let capabilities =
    ({
         ...
         (fun connection database metadata ->
           {
             ...
             skip_conn =
               Melange_datascript_spec.Api.connection_skips_validation
       ...
       validate_report =
         (fun report ->
           let result =
             Validation_schema.validateTransactionValueWith runtime datascript
               report transaction_validation_options
           in
           if result.valid then Ok () else Error result.errors);
       same_database = ( == );
       report_after = Melange_datascript_spec.Api.report_db_after datascript;
       report_has_datoms =
         (fun report ->
           Array.length
             (Melange_datascript_spec.Api.report_datoms datascript report)
           > 0);
       compare_and_set =
         Melange_datascript_spec.Api.compare_and_set_database datascript;
       store_after = Melange_datascript_spec.Api.store_after_transact datascript;
       run_callbacks = Melange_datascript_spec.Api.run_callbacks datascript;
       direct_transact =
         (fun connection transaction_data metadata ->
           Melange_datascript_spec.Api.transact datascript connection
             transaction_data
             (Js.Nullable.return metadata));
