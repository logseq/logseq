...
type encoded_value_block_options = {
  blockUuid : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t;
  properties : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t;
}

type encoded_closed_value_options = {
  dbIdent : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t;
  icon : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t;
}
    ...
    then false
      ...
      | [| attribute; uuid |] ->
          Melange_cljs_runtime_spec.Value_codec.value_equals runtime attribute
            block_uuid
          && Melange_cljs_runtime_spec.Value_codec.value_is_uuid runtime uuid
      | _ -> false
  in
  let block_with_uuid value =
    Melange_cljs_runtime_spec.Value_codec.value_is_map runtime value
    && Melange_cljs_runtime_spec.Value_codec.map_get runtime value block_uuid
       |> Melange_cljs_runtime_spec.Value_codec.value_is_uuid runtime
  in
  let project value =
    let collection =
...
let timestamp_block runtime (now_ms : float_callback) block =
  ...
  let timestamps =
    Melange_common.Util.ensure_block_timestamps ~now_ms:now
      ~created_at:
        (if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime created
         then None
         else
           Some
             (Melange_cljs_runtime_spec.Value_codec.float_from_value runtime
                created))
      ~updated_at:
        (if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime updated
         then None
         else
           Some
             (Melange_cljs_runtime_spec.Value_codec.float_from_value runtime
...
let buildClosedValueBlockWith runtime (now_ms : float_callback) block_uuid
  ...
  if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime block_uuid then
  ...
  let result =
    ...
    | Some ident
      when Melange_cljs_runtime_spec.Value_codec.value_is_keyword runtime ident
      ->
  ...
  let result =
    ...
    | Some icon
      when Melange_cljs_runtime_spec.Value_codec.value_truthy runtime icon ->
...
let buildClosedValuesWith runtime (generate_order : value_callback)
    ...
    (get_property_schema :
      Melange_cljs_runtime_spec.Value_codec.callback Js.Nullable.t)
    (build_new_property :
      Melange_cljs_runtime_spec.Value_codec.callback Js.Nullable.t) =
  let property_schema =
    ...
    | Resolve_property_schema -> (
        ...
        | Some callback ->
            Melange_cljs_runtime_spec.Value_codec.invoke_callback runtime
              callback property
...
let build_property_value_entries_with runtime (generate_uuid : value_callback)
  ...
  let value_block_options raw =
    let properties =
      if not options.pvalueMap then None
        ...
        if
          Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime attributes
          || Array.length
               (Melange_cljs_runtime_spec.Value_codec.map_to_entries runtime
                  attributes)
             = 0
  ...
  let project raw_value property =
    let raw_items =
      if Melange_cljs_runtime_spec.Value_codec.value_is_set runtime raw_value
      then Melange_cljs_runtime_spec.Value_codec.set_to_array runtime raw_value
      else [||]
    in
    let collection =
      Array.length raw_items > 0
      || Melange_cljs_runtime_spec.Value_codec.value_is_set runtime raw_value
    in
    let value =
      if collection then
      ...
      Domain.plan_property_value
        {
          ...
          all_values_uuid =
            ...
            && Array.for_all
                 ...
                 value_items;
          single_value_uuid =
            Melange_cljs_runtime_spec.Value_codec.value_is_uuid runtime value;
        }
    with
    | Domain.Uuid_set ->
        value_items
  ...
  let entries =
    ...
    |> Array.map (function
      | [| property_key; raw_value |] ->
          ...
          then invalid_arg "Property value map key requires :db/ident";
          let output_key =
