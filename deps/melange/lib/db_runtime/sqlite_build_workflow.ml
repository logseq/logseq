type value_callback =
  (unit -> Melange_cljs_runtime_spec.Value_codec.cljs_value[@u])
...
module Default_capabilities = struct
  ...
  let uuid_text () =
    ...
    | None ->
        invalid_arg "SQLite build UUID generation requires crypto.randomUUID"
end
...
let dissoc runtime name map =
  Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime map
...
let select_many runtime names map =
  ...
  |> Rrbvec.fold_left
       (fun result name ->
         ...
         if Melange_cljs_runtime_spec.Value_codec.map_contains runtime map key
         then
           Melange_cljs_runtime_spec.Value_codec.map_assoc runtime result key
             (Melange_cljs_runtime_spec.Value_codec.map_get runtime map key)
         else result)
       (empty_map runtime)
...
let require_map runtime label value =
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_map runtime value) then
...
let require_vector runtime label value =
  ...
  then fail (label ^ " must be a vector")
...
let validate_keyword_collection runtime label value =
  ...
  |> Rrbvec.iter (fun item ->
      if
        not
          (Melange_cljs_runtime_spec.Value_codec.value_is_keyword runtime item)
      then fail (label ^ " values must be keywords"))

let validate_properties_map runtime label value =
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime value) then (
...
let rec validate_block runtime block =
  ...
  then fail "block :block/title must be a string";
  require_optional runtime "block/uuid"
    Melange_cljs_runtime_spec.Value_codec.value_is_uuid "a UUID"
  ...
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime tags) then
    validate_keyword_collection runtime ":build/tags" tags;
  require_optional runtime "build/keep-uuid?"
    Melange_cljs_runtime_spec.Value_codec.value_is_bool "a boolean"
    (field runtime block "build/keep-uuid?");
  let children = field runtime block "build/children" in
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime children)
  then (
...
let validate_page runtime page =
  ...
  if
    ...
    && Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime uuid
    && Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime journal
  then fail "page requires :block/title, :block/uuid, or :build/journal";
  require_optional runtime "block/title"
    Melange_cljs_runtime_spec.Value_codec.value_is_string "a string" title;
  require_optional runtime "block/uuid"
    Melange_cljs_runtime_spec.Value_codec.value_is_uuid "a UUID" uuid;
  require_optional runtime "build/journal"
    Melange_cljs_runtime_spec.Value_codec.value_is_integer "an integer" journal;
  validate_properties_map runtime ":build/properties"
...
let validate_page_entry runtime entry =
  ...
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime blocks)
  then (
...
let validate_property_definition runtime definition =
  ...
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime ref_types)
  then (
    ...
    |> Array.iter (function
      | [| key; value |]
        when Melange_cljs_runtime_spec.Value_codec.value_is_keyword runtime key
             && Melange_cljs_runtime_spec.Value_codec.value_is_keyword runtime
                  value ->
          ()
  ...
  if
    not
      (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime closed_values)
  then (
    ...
    |> Rrbvec.iter (fun closed_value ->
        ...
        require_optional runtime "uuid"
          Melange_cljs_runtime_spec.Value_codec.value_is_uuid "a UUID"
          (field runtime closed_value "uuid");
        require_optional runtime "icon"
          Melange_cljs_runtime_spec.Value_codec.value_is_map "a map"
          (field runtime closed_value "icon")));
  let classes = field runtime definition "build/property-classes" in
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime classes)
  then validate_keyword_collection runtime ":build/property-classes" classes;
  require_optional runtime "build/keep-uuid?"
    Melange_cljs_runtime_spec.Value_codec.value_is_bool "a boolean"
...
let validate_class_definition runtime definition =
  ...
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime extends)
  then validate_keyword_collection runtime ":build/class-extends" extends;
  let parent = field runtime definition "build/class-parent" in
  require_optional runtime "build/class-parent"
    Melange_cljs_runtime_spec.Value_codec.value_is_keyword "a keyword" parent;
  let properties = field runtime definition "build/class-properties" in
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime properties)
  then (
...
let validate_definition_map runtime label validate_definition definitions =
  if
    not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime definitions)
  then (
...
let validateOptionsWith runtime options =
  ...
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime pages) then (
  ...
  require_optional runtime "graph-namespace"
    Melange_cljs_runtime_spec.Value_codec.value_is_keyword "a keyword"
...
let default_option runtime name value options =
  let key = keyword runtime name in
  if Melange_cljs_runtime_spec.Value_codec.map_contains runtime options key then
    options
  else Melange_cljs_runtime_spec.Value_codec.map_assoc runtime options key value
...
let create_ident_with runtime stable_idents random_index random_bytes ~kind
  ...
  | None, Some _ -> source
  | _ ->
      Melange_db.Db_ident.create_with ~stable_idents ~random_index ~random_bytes
...
let get_ident runtime all_idents value =
  if qualified_keyword runtime value then value
    ...
    let ident =
      Melange_cljs_runtime_spec.Value_codec.map_get runtime all_idents value
    in
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime ident then
      Js.Exn.raiseError
        ("No ident found for "
        ^ Melange_cljs_runtime_spec.Value_codec.value_to_string runtime value)
    else ident
...
let prepare_page runtime capabilities page =
  let journal = field runtime page "build/journal" in
  let page =
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime journal then
      page
    else
      let journal_day =
        Melange_cljs_runtime_spec.Value_codec.int_from_value runtime journal
      in
      let existing_uuid = field runtime page "block/uuid" in
      let new_page =
        Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime existing_uuid
      in
      let uuid =
        if new_page then
      ...
      |> assoc runtime "logseq.db.sqlite.build/new-page?"
           (Melange_cljs_runtime_spec.Value_codec.bool_to_value runtime new_page)
  in
  let uuid = field runtime page "block/uuid" in
  ...
  if not extract then pages
    ...
    let existing =
      ...
      |> Rrbvec.filter_map (fun entry ->
          ...
          in
          if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime title
          then None
          else
            Some
           ...
           | [| _property; values |] ->
               ...
               |> Rrbvec.fold_left
                    (fun result value ->
                      let values =
                        if
                          Melange_cljs_runtime_spec.Value_codec.value_is_set
                            runtime value
                        then
                          Melange_cljs_runtime_spec.Value_codec.set_to_array
                            runtime value
                          |> Rrbvec.of_array
                      ...
                      |> Rrbvec.fold_left
                           (fun result value ->
                             match
                               Sqlite_build.page_property_value runtime value
                             with
                             | None -> result
                             | Some page ->
  ...
  Rrbvec.fold_left
    (fun result entry ->
      ...
      if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime title then
        ...
        Melange_cljs_runtime_spec.Value_codec.map_assoc runtime result title
...
let build_page runtime capabilities options page =
  ...
  let has_changes =
    ...
    let tags = field runtime page "build/tags" in
    (not
       (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime properties))
    || not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime tags)
  in
                 ...
                 if named then
                   let value =
                     Melange_cljs_runtime_spec.Value_codec.map_get runtime
                       page_uuids
                       (Melange_cljs_runtime_spec.Value_codec.string_to_value
                          runtime name)
                   in
                   if
                     Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime
                       value
                   then Js.Exn.raiseError ("No uuid for page ref name " ^ name)
                   else
                     Melange_cljs_runtime_spec.Value_codec.uuid_to_string
                       runtime value
                 else name
               in
               let reference =
...
let build_block runtime capabilities options all_idents page_uuids page_id block
  ...
  let result =
    if existing then
      ...
      let parent =
        if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime parent
        then id_reference runtime page_id
        else parent
      in
      empty_map runtime
      |> assoc runtime "db/id" (next_temp_id runtime)
  ...
  in
  let result =
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime tags then
      result
    else
...
let schema_for_property runtime definition =
  ...
  |> Rrbvec.fold_left
       (fun result name ->
         let key = keyword runtime name in
         if
           Melange_cljs_runtime_spec.Value_codec.map_contains runtime definition
             key
         then
           Melange_cljs_runtime_spec.Value_codec.map_assoc runtime result key
             (Melange_cljs_runtime_spec.Value_codec.map_get runtime definition
                key)
         else result)
       (empty_map runtime)
  ...
  then None
    ...
    |> Rrbvec.find_map (fun entry ->
        ...
        then
          ...
          if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime uuid
          then None
          else Some uuid
        else None)
...
let lookup_reference runtime value =
  ...
  then false
    ...
    with
    | [| marker; _uuid |] ->
        Melange_cljs_runtime_spec.Value_codec.value_equals runtime marker
  ...
  if not translate then None
    ...
    let title =
      ...
      if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime journal then
        ...
        journal |> Melange_cljs_runtime_spec.Value_codec.int_from_value runtime
        |> fun journal_day ->
        Melange_common.Date_time.format_journal_day ~journal_day
    ...
    in
    let uuid =
      Melange_cljs_runtime_spec.Value_codec.map_get runtime page_uuids title
    in
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime uuid then
      Js.Exn.raiseError
        ("No uuid for page "
        ^ Melange_cljs_runtime_spec.Value_codec.value_to_string runtime title)
    else
      Some
        (Melange_cljs_runtime_spec.Value_codec.array_to_vector runtime
...
let rec build_property_values runtime capabilities options all_idents properties
  ...
  let build_one property_name value =
    ...
    let definition =
      Melange_cljs_runtime_spec.Value_codec.map_get runtime properties
        property_name
    in
    let property_type =
          ...
          when Sqlite_build.block_property_value runtime value
            ...
            let uuid =
              ...
              if
                Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime
                  value_uuid
              then capabilities.generateUuid () [@u]
              else value_uuid
            in
            let lookup =
              Melange_cljs_runtime_spec.Value_codec.array_to_vector runtime
            ...
            let nested_transactions, nested_properties =
              ...
              if
                Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime
                  values
              then (Rrbvec.empty, empty_map runtime)
              else
            ...
            let attributes =
              ...
              if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime tags
              then attributes
              else
                tags |> collection runtime
                |> Rrbvec.map (fun tag ->
...
let ensure_lookup_id runtime entity =
  ...
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime id) then
    entity
  else
...
let add_page_tags runtime all_idents source page =
  ...
  if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime tags then page
  else
    let tag_idents =
  ...
  |> Rrbvec.concat_map (function
    | [| property_name; definition |] ->
        ...
        then Rrbvec.empty
          ...
          in
          let property =
            build_property runtime capabilities.generateOrder capabilities.nowMs
              db_ident schema options_map
            |> assoc runtime "db/id" (next_temp_id runtime)
          in
          let property =
            let order =
              Melange_cljs_runtime_spec.Value_codec.map_get runtime
                class_property_orders property_name
            in
            if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime order
            then property
          ...
          let property =
            let classes = field runtime definition "build/property-classes" in
            if
              Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime classes
            then property
            else
...
let prepare_closed_values runtime capabilities properties =
  ...
  |> Array.map (function
    | [| property_name; definition |] ->
        ...
        let definition =
          if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime values
          then definition
          else
            values |> collection runtime
            |> Rrbvec.map (fun entry ->
  ...
  |> Rrbvec.map (function
    | [| class_name; definition |] ->
        ...
        let parents =
          if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime parent
          then
            field runtime definition "build/class-extends" |> collection runtime
  ...
  let class_ids =
    ...
    |> Rrbvec.fold_left
         (fun result -> function
           | [| class_name; _ |] ->
               Melange_cljs_runtime_spec.Value_codec.map_assoc runtime result
                 class_name (next_temp_id runtime)
           | _ -> result)
         (empty_map runtime)
  in
  let class_reference class_name =
    let id =
      Melange_cljs_runtime_spec.Value_codec.map_get runtime class_ids class_name
    in
    if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime id) then
      id
    else
  ...
  |> Rrbvec.concat_map (function
    | [| class_name; class_definition |] ->
        ...
        let block =
          ...
          |> assoc runtime "db/ident" db_ident
          |> assoc runtime "db/id"
               (Melange_cljs_runtime_spec.Value_codec.map_get runtime class_ids
                  class_name)
...
let build_blocks_tx_with build_all_idents build_properties runtime capabilities
    options =
  ...
  let all_idents = build_all_idents runtime options properties classes in
...
let buildBlocksTx runtime options =
  build_blocks_tx_with_capabilities runtime
...
let create_blocks_with build_blocks_tx runtime datascript capabilities
  ...
  ignore
    (Melange_datascript_spec.Api.transact datascript connection init
       Js.Nullable.null);
  if
    Array.length
  ...
  then Js.Nullable.null
    ...
    Melange_datascript_spec.Api.transact datascript connection deferred
      Js.Nullable.null
    |> Js.Nullable.return
...
let createBlocksInput runtime datascript connection input =
  let options =
    if Melange_cljs_runtime_spec.Value_codec.value_is_vector runtime input then
      empty_map runtime |> assoc runtime "pages-and-blocks" input
    else input
  in
  createBlocks runtime datascript
    (default_capabilities runtime)
    connection options
