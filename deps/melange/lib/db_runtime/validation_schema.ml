...
type encoded_transaction_value_result = {
  valid : bool;
  errors : Melange_cljs_runtime_spec.Value_codec.cljs_value;
}

type encoded_database_result = {
  datomCount : int;
  entities : Melange_cljs_runtime_spec.Value_codec.cljs_value array;
  errors : encoded_entity_error array;
}
...
let tuple_component runtime tuple index =
  let values =
    Melange_cljs_runtime_spec.Value_codec.collection_to_array runtime tuple
  in
...
let property_tuple_error_message runtime datascript tuple =
  ...
  in
  if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime property_type
  then Melange_db.Validation_property.error_message ""
  else
    property_type
...
let workflow_capabilities runtime datascript database
  ...
  in
  ({
     map_entries = Melange_cljs_runtime_spec.Value_codec.map_to_entries runtime;
     field = Entity_read.field runtime datascript;
     has_field =
       (fun value name ->
         Melange_cljs_runtime_spec.Value_codec.map_contains runtime value
           (keyword name));
     assoc_field =
     ...
     is_integer = Melange_cljs_runtime_spec.Value_codec.value_is_integer runtime;
...
let encode_entity_error
    (error :
      Melange_cljs_runtime_spec.Value_codec.cljs_value Domain.entity_error) =
...
let validateDatabaseWith runtime datascript database
  ...
  let capabilities =
    ({
       ...
       remove_field =
         (fun entity field ->
           Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime entity
             (Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime
                field));
       prepare_entities =
         (fun database entities ->
           Validation_property.prepareEntitiesWith runtime datascript database
             entities);
       validate_entities =
         (fun database entities ->
...
let log_transaction_errors runtime changed_ids tx_metadata errors =
  Melange_cljs_runtime_spec.Value_codec.log_values runtime
    [|
      Melange_cljs_runtime_spec.Value_codec.string_to_value runtime
    ...
    (fun (error :
      ...
      let diagnostic =
        Melange_cljs_runtime_spec.Value_codec.entries_to_map runtime
          ...
          |]
      in
      Melange_cljs_runtime_spec.Value_codec.log_values runtime [| diagnostic |])
    errors
...
let validateTransactionWith runtime datascript report
  ...
  let capabilities =
    ({
       report_db_after = Melange_datascript_spec.Api.report_db_after datascript;
       report_datoms = Melange_datascript_spec.Api.report_datoms datascript;
       report_tx_metadata =
         Melange_datascript_spec.Api.report_tx_metadata datascript;
