...
type value_predicate =
  (Melange_cljs_runtime_spec.Value_codec.cljs_value -> bool[@u])
...
type runtime_page_group = {
  title : Melange_cljs_runtime_spec.Value_codec.cljs_value;
  journal : Melange_cljs_runtime_spec.Value_codec.cljs_value;
  entries : Melange_cljs_runtime_spec.Value_codec.cljs_value Rrbvec.t ref;
}

type encoded_import_validation_result = {
  database : Melange_datascript_spec.Api.database Js.Nullable.t;
  transactionData : Melange_cljs_runtime_spec.Value_codec.cljs_value;
...
let datom_export runtime export_map =
  ...
  in
  (not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime format))
...
let page_entity runtime datascript database page =
  ...
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime journal)
  then
    let datoms =
    ...
    if Array.length datoms = 0 then None
      ...
      Melange_datascript_spec.Api.datom_entity datascript datoms.(0)
      |> entity runtime datascript database
  else
    let title = field runtime page "block/title" in
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime title then
      None
    else
...
let filter_existing_properties runtime datascript entity properties =
  ...
  |> Array.fold_left
       (fun result -> function
         | [| property; value |] ->
             let existing =
               Melange_datascript_spec.Api.entity_get datascript entity property
             in
             if
...
let prepare_page runtime datascript database keep_existing_properties
  ...
  | Some existing ->
      ...
      if
        not
          (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime old_uuid)
      then
        replacements :=
          Melange_cljs_runtime_spec.Value_codec.map_assoc runtime !replacements
...
let rec postwalk runtime transform value =
  let walked =
    ...
    else if Melange_cljs_runtime_spec.Value_codec.value_is_vector runtime value
    then
      value
      ...
      then value
        ...
        | [| marker; uuid |]
          when Melange_cljs_runtime_spec.Value_codec.value_equals runtime marker
            ...
            let replacement =
              Melange_cljs_runtime_spec.Value_codec.map_get runtime replacements
                uuid
            in
            if
              Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime
...
let update_existing_definitions runtime datascript database definitions =
  if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime definitions then
    empty_map runtime
  else
    definitions
    |> Melange_cljs_runtime_spec.Value_codec.map_to_entries runtime
...
let current_block_import runtime datascript export_map current_block =
  ...
  if
    Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime exported
    || Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime current_block
  then export_map
  else
    ...
    let page_entry =
      ...
      |> assoc runtime "blocks"
           (Melange_cljs_runtime_spec.Value_codec.array_to_vector runtime
              [|
                Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime block
...
let internal_property runtime property =
  Melange_db.Property_identity.is_internal_property
    ...
    ~is_keyword:
      (Melange_cljs_runtime_spec.Value_codec.value_is_keyword runtime property)
...
let include_namespace_properties runtime datascript database export_map =
  ...
  if Rrbvec.is_empty prefixes then export_map
                       ...
                       |> Rrbvec.fold_left
                            (fun definition name ->
                              ...
                              if
                                ...
                                .value_is_nil runtime value
                              then definition
                              else assoc runtime name value definition)
                            (empty_map runtime)
                     in
                     let current =
                       Melange_cljs_runtime_spec.Value_codec.map_get runtime
                         result property
                     in
                     let definition =
                       if
                         Melange_cljs_runtime_spec.Value_codec.value_is_nil
                           runtime current
                       then definition
    ...
    Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime export_map
...
let add_distinct runtime values value =
  if
    Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime value
    || Rrbvec.exists
...
let nonempty_map runtime value =
  (not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime value))
  && Array.length
...
let buildTagsWith runtime datascript block_tags =
  ...
  |> Rrbvec.fold_left
       (fun result tag ->
         ...
         if
           Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime ident
...
let propertyValueContentWith runtime datascript entity =
  ...
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime title) then
    title
...
let closedValuesForExportWith runtime datascript database property =
  ...
  let referenced_reverse =
    ...
    |> Rrbvec.filter (fun value ->
        ...
        Rrbvec.exists
          (Melange_cljs_runtime_spec.Value_codec.value_equals runtime content)
          referenced)
  in
  let values =
    ...
    let result =
      if not keep_uuid then result
        ...
        |> assoc runtime "build/keep-uuid?"
             (Melange_cljs_runtime_spec.Value_codec.bool_to_value runtime true)
    in
...
let buildNodeClassesWith runtime datascript database build_block block_tags
  ...
  let property_values map =
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime map then
      Rrbvec.empty
    else
      map
      |> Melange_cljs_runtime_spec.Value_codec.map_to_entries runtime
  ...
  let pvalue_classes =
    ...
    |> Rrbvec.fold_left
         (fun result value_or_values ->
           let values =
             if
               Melange_cljs_runtime_spec.Value_codec.value_is_set runtime
                 value_or_values
             then
               Melange_cljs_runtime_spec.Value_codec.set_to_array runtime
                 value_or_values
                ...
                (fun result value ->
                  let tags =
                    if Sqlite_build.pagePropertyValueWith runtime value then
                      match
                        Melange_cljs_runtime_spec.Value_codec.vector_to_array
                          runtime value
                      with
                      | [| _marker; page |] -> field runtime page "build/tags"
                      | _ ->
                          Melange_cljs_runtime_spec.Value_codec.nil_value
                            runtime
                    else if Sqlite_build.blockPropertyValueWith runtime value
  ...
  |> Rrbvec.fold_left
       (fun result entity ->
         ...
         in
         Melange_cljs_runtime_spec.Value_codec.map_assoc runtime result ident
...
let dissoc_names runtime names value =
  ...
  |> Rrbvec.fold_left
       (fun result name ->
         Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime result
...
let entity_required runtime datascript database lookup label =
  ...
  | None ->
      invalid_arg
        ...
        ^ Melange_cljs_runtime_spec.Value_codec.value_to_string runtime lookup)
       ...
       (fun result value_or_values ->
         ...
         |> Rrbvec.fold_left
              (fun result value ->
                if
                  not
                    (Melange_cljs_runtime_spec.Value_codec.value_is_vector
                       runtime value)
                then result
                else
                  match
                  ...
                  with
                  | [| attribute; uuid |]
                    when Melange_cljs_runtime_spec.Value_codec.value_equals
                           runtime attribute
                           (keyword runtime "block/uuid") ->
                      let metadata =
                        Melange_cljs_runtime_spec.Value_codec.value_meta runtime
                          value
                      in
                      if
                        Melange_cljs_runtime_spec.Value_codec.value_is_nil
                        ...
                        || (not
                              (Melange_cljs_runtime_spec.Value_codec
                               .map_contains runtime metadata marker))
                        || not
                             (Melange_cljs_runtime_spec.Value_codec.map_get
                                runtime metadata marker
                             |> Melange_cljs_runtime_spec.Value_codec
                                .value_truthy runtime)
                      then result
...
let rec buildable_properties runtime datascript database predicates
  ...
  let closed_lookup property value =
    let definition =
      Melange_cljs_runtime_spec.Value_codec.map_get runtime properties_config
        property
    in
    let entries =
  ...
  and build_pvalue_default property_entity pvalue =
    ...
    let reference_type =
      ...
      |> Rrbvec.exists (fun name ->
          Melange_cljs_runtime_spec.Value_codec.value_equals runtime
            property_type (keyword runtime name))
    in
    let default_property =
      Melange_cljs_runtime_spec.Value_codec.value_equals runtime property_ident
    ...
    if reference_type && not default_property then
      if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime ident)
      then ident
      else
        ...
        let metadata =
          ...
          |> Melange_cljs_runtime_spec.Value_codec.map_assoc runtime
               ...
               (Melange_cljs_runtime_spec.Value_codec.bool_to_value runtime true)
        in
        Melange_cljs_runtime_spec.Value_codec.value_with_meta runtime lookup
          metadata
    else if
      not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime ident)
    then ident
    else
  ...
  |> Rrbvec.fold_left
       (fun result (property, value) ->
         let first_value =
           if Melange_cljs_runtime_spec.Value_codec.value_is_set runtime value
           then
           ...
           if closed then
             ...
             then
               ...
               Melange_cljs_runtime_spec.Value_codec.array_to_vector runtime
                 ...
                 |]
           else if Melange_datascript_spec.Api.entity_is datascript value then
             let property_entity =
         ...
         in
         Melange_cljs_runtime_spec.Value_codec.map_assoc runtime result property
           built)
       (empty_map runtime)
    ...
    |> Rrbvec.map (fun ident ->
        ...
        let definition =
          if not include_uuid then definition
            ...
            |> assoc runtime "build/keep-uuid?"
                 (Melange_cljs_runtime_spec.Value_codec.bool_to_value runtime
                    true)
        in
        let definition =
...
and build_node_properties runtime datascript database predicates entity
  ...
  let configured ident =
    (not
       (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime
          existing_properties))
    && Melange_cljs_runtime_spec.Value_codec.map_contains runtime
         existing_properties ident
  in
  let rec collect_nested_property_ids result value =
    if Melange_datascript_spec.Api.entity_is datascript value then
      let created_from =
        entity_field runtime datascript value
          "logseq.property/created-from-property"
      in
      if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime created_from
      then result
      else
        let properties =
...
and build_node_export runtime datascript database predicates entity options =
  ...
  let new_properties =
    if shallow_copy || exclude_ontology then empty_map runtime
      ...
      let property_options =
        Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime options
  ...
  let properties_config =
    ...
    let configured =
      if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime configured
      then empty_map runtime
      else configured
    in
...
and build_blocks_export runtime datascript database predicates blocks options =
  ...
  let include_children =
    ...
    if Melange_cljs_runtime_spec.Value_codec.map_contains runtime options key
    then
      Melange_cljs_runtime_spec.Value_codec.map_get runtime options key
      |> Melange_cljs_runtime_spec.Value_codec.value_truthy runtime
    else true
  in
  ...
  let children_of block =
    ...
    |> Rrbvec.find_opt (fun (parent, _children) ->
        Melange_cljs_runtime_spec.Value_codec.value_equals runtime parent id)
    |> Option.map snd
    |> Option.value ~default:Rrbvec.empty
  in
  ...
  let rec build_block block =
    ...
    let node_options =
      ...
      |> fun options ->
      Melange_cljs_runtime_spec.Value_codec.map_assoc runtime options
  ...
  let result =
    if
      Melange_cljs_runtime_spec.Value_codec.value_equals runtime !properties
        raw_initial_properties
    then result
    else assoc runtime "properties" !properties result
  in
  if
    Melange_cljs_runtime_spec.Value_codec.value_equals runtime !classes
      raw_initial_classes
  then result
...
let datoms_for_attribute runtime datascript database name =
  ...
  Melange_datascript_spec.Api.datoms datascript database
    (keyword runtime "avet") [| attribute |]
             ...
             |> Rrbvec.fold_left
                  (fun result id ->
                    add_distinct runtime result
                      (Melange_cljs_runtime_spec.Value_codec.uuid_from_string
                         runtime id))
                  result)
         Rrbvec.empty
  in
             ...
             | Some source -> (
                 ...
                 | Some page ->
                     ...
                     |> Option.fold ~none:true ~some:(fun value ->
                         not
                           (Melange_cljs_runtime_spec.Value_codec.value_truthy
                              runtime value)))
         in
...
let graphFilesWith runtime datascript database include_timestamps =
  let files =
    ...
    |> Array.map (fun datom ->
        let source =
          Melange_datascript_spec.Api.datom_entity datascript datom |> fun id ->
  ...
  Array.stable_sort
    (fun left right ->
      String.compare
        ...
        |> Melange_cljs_runtime_spec.Value_codec.string_from_value runtime))
    files;
  Melange_cljs_runtime_spec.Value_codec.array_to_vector runtime files
       ...
       (fun result -> function
         | [| key; value |] ->
             ...
             then
               ...
               let existing =
                 Melange_cljs_runtime_spec.Value_codec.map_get runtime result
                   key
               in
               let merged =
                 if
                 ...
                 then merge_map runtime existing value
                   ...
                   Array.append
                     ...
                     (Melange_cljs_runtime_spec.Value_codec.collection_to_array
                        runtime value)
                   |> Melange_cljs_runtime_spec.Value_codec.array_to_vector
                        runtime
               in
               Melange_cljs_runtime_spec.Value_codec.map_assoc runtime result
                 key merged
              ...
              (fun result -> function
                | [| key; definition |] ->
                    let definition =
                      if
                        not
                          (Melange_cljs_runtime_spec.Value_codec.map_contains
                             runtime result key)
                      then definition
                      else
                        merge_map runtime
...
let mergeExportMapsWith runtime export_maps =
  ...
  let rec find_group title journal index =
    if index >= Rrbvec.length !groups then None
      ...
      if
        Melange_cljs_runtime_spec.Value_codec.value_equals runtime group.title
          title
        && Melange_cljs_runtime_spec.Value_codec.value_equals runtime
  ...
  let result =
    if
      Array.length
        (Melange_cljs_runtime_spec.Value_codec.map_to_entries runtime properties)
      = 0
    then result
    else assoc runtime "properties" properties result
  in
  if
    Array.length
      (Melange_cljs_runtime_spec.Value_codec.map_to_entries runtime classes)
    = 0
  then result
  else assoc runtime "classes" classes result
...
let options_with_flags runtime flags =
  ...
  |> Rrbvec.fold_left
       (fun result (name, enabled) ->
         assoc runtime name
           (Melange_cljs_runtime_spec.Value_codec.bool_to_value runtime enabled)
           result)
       (empty_map runtime)
...
let build_uuid_block_export runtime datascript database pvalue_uuids
  ...
  |> Rrbvec.iter (fun block ->
      ...
      let excluded =
        Option.fold ~none:false
          ~some:
            (Melange_cljs_runtime_spec.Value_codec.value_equals runtime page)
          excluded_page
      in
...
let finalize_export_maps runtime datascript database export_maps =
  ...
  if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime pages then
    merged
  else
    assoc runtime "pages-and-blocks"
...
let buildBlockExportWith runtime datascript database eid =
  ...
  let property_values =
    ...
    |> Rrbvec.filter (Melange_datascript_spec.Api.entity_is datascript)
  in
  ...
  let predicates : export_predicates =
    {
      includeUuid =
        (fun[@u] uuid ->
          Rrbvec.exists
            (Melange_cljs_runtime_spec.Value_codec.value_equals runtime uuid)
            content_uuids);
      includePvalueUuid = (fun[@u] _value -> false);
    }
  in
  ...
  |> Rrbvec.fold_left
       (fun result alias ->
         ...
         then result
           ...
           match
             Melange_cljs_runtime_spec.Value_codec.vector_to_array runtime alias
           with
        ...
        if Rrbvec.is_empty values then ""
          ...
          let rendered =
            ...
            |> Rrbvec.fold_left
                 (fun result value ->
                   let value =
                     Melange_cljs_runtime_spec.Value_codec.value_to_string
                       runtime value
                   in
...
let referenced_uuid runtime references uuid =
  (not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime uuid))
  && Rrbvec.exists
       (Melange_cljs_runtime_spec.Value_codec.value_equals runtime uuid)
       references

let map_update_values runtime value transform =
  if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime value then value
  else
    value
    |> Melange_cljs_runtime_spec.Value_codec.map_to_entries runtime
    |> Array.map (function
...
let rec prune_export_entity runtime references value =
  ...
  let value =
    if referenced_uuid runtime references uuid then value
      ...
      let value =
        Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime value
          (keyword runtime "block/uuid")
      in
      Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime value
...
let rec prune_export_blocks runtime references blocks =
  ...
  |> Rrbvec.map (fun block ->
      ...
      if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime children
      then block
      else
        assoc runtime "build/children"
  ...
  if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime pages then
    ...
    let pages =
      ...
      |> Rrbvec.map (fun entry ->
          ...
          let entry =
            if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime blocks
            then entry
            else
              assoc runtime "blocks"
...
let build_page_blocks_export runtime datascript database predicates page options
  ...
  let built_page =
    if option_bool runtime options "ontology-page?" then node
      ...
      let node_without_title =
        Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime node
          (keyword runtime "block/title")
      in
      let result =
  ...
  let blocks =
    ...
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime blocks then
      Melange_cljs_runtime_spec.Value_codec.array_to_vector runtime [||]
    else blocks
  in
  let page_entry =
...
let pvalue_descendant runtime datascript block =
  let rec loop parent =
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime parent then
      false
    else
      let created_from =
        entity_field runtime datascript parent
...
let build_page_export_core runtime datascript database predicates included_uuids
  ...
  let class_page_properties =
    if ontology_page || not (Entity_read.classWith runtime datascript page) then
      Melange_cljs_runtime_spec.Value_codec.nil_value runtime
    else
      let property_idents =
  ...
  let page_options =
    if
      Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime ontology_export
    then blocks_export
    else
      Melange_cljs_runtime_spec.Value_codec.array_to_vector runtime
...
let buildPageExportWith runtime datascript database eid =
  ...
  in
  let alias_export =
    if Rrbvec.is_empty aliases then
      Melange_cljs_runtime_spec.Value_codec.nil_value runtime
    else
      let pages =
        aliases
  ...
  |> Rrbvec.iter (fun block ->
      ...
      if
        ...
        |> Rrbvec.exists (fun (existing, _blocks) ->
            Melange_cljs_runtime_spec.Value_codec.value_equals runtime existing
              page)
      then
        block_groups :=
          !block_groups
    ...
    |> Rrbvec.fold_left
         (fun result node ->
           if entity_is_page runtime datascript node then result
             ...
             in
             let descendants =
               Tree_workflow.blockAndChildrenWith runtime datascript database
                 uuid false
               |> Js.Nullable.toOption |> Option.map Rrbvec.of_array
               |> Option.value ~default:Rrbvec.empty
             in
...
let lookup_uuid_value runtime value =
  ...
  then None
    ...
    with
    | [| attribute; uuid |]
      when Melange_cljs_runtime_spec.Value_codec.value_equals runtime attribute
             (keyword runtime "block/uuid") ->
        Some uuid
    | _ -> None
...
let buildExportWith runtime datascript database
  ...
  let export =
    ...
    | "graph" ->
        Sqlite_export.graphDatomsWith runtime datascript database schema_version
    | "graph-human" ->
...
let diffExportsWith runtime capabilities export_map other_export_map =
  ...
  if not (Melange_cljs_runtime_spec.Value_codec.value_is_vector runtime diff)
  then invalid_arg "SQLite export diff capability must return a vector";
  match Melange_cljs_runtime_spec.Value_codec.vector_to_array runtime diff with
  | [| left; right; _common |] ->
      if
        Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime left
        && Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime right
      then Melange_cljs_runtime_spec.Value_codec.nil_value runtime
      else
...
let import_validation_error runtime message =
  ({
     ...
     transactionData = Melange_cljs_runtime_spec.Value_codec.nil_value runtime;
...
let log_validation_errors runtime edn_label error_count errors =
  ...
  |> Melange_cljs_runtime_spec.Value_codec.array_to_vector runtime
  |> fun diagnostics ->
  Melange_cljs_runtime_spec.Value_codec.log_values runtime [| diagnostics |]
...
let validateImportTransactionsWith runtime datascript transactions database
  ...
  if
    not
      (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime supplied_error)
  then
          ...
          dry_run =
            (fun database transactions ->
              let transaction_data =
                ...
                |> Melange_cljs_runtime_spec.Value_codec.array_to_vector runtime
              in
              Melange_datascript_spec.Api.with_tx datascript database
                transaction_data Js.Nullable.undefined
              |> Melange_datascript_spec.Api.report_db_after datascript);
          validate =
            (fun database ->
              let validation =
                Validation_schema.validateLocalDatabaseWith runtime datascript
                  database validation_options false
              in
                   ...
                   (fun (error : Validation_schema.encoded_entity_error) ->
                     ...
                     ({
                        ...
                        groups =
                          ...
                          |> Rrbvec.map
                               (fun
                                 (group : Validation_schema.encoded_error_group)
                               ->
                                 ({
                                    attribute =
    ...
    if not (Rrbvec.is_empty conflicts) then
      ...
      let prepared =
        ...
        |> assoc runtime "extract-content-refs?"
             (Melange_cljs_runtime_spec.Value_codec.bool_to_value runtime false)
      in
      let prepared =
...
let createSeededConnectionWith datascript schema initial_data =
  let connection =
    Melange_datascript_spec.Api.create_conn datascript schema
      Js.Nullable.undefined
  in
  let _report =
    Melange_datascript_spec.Api.transact datascript connection initial_data
      Js.Nullable.undefined
  in
  connection
...
let validateExport runtime datascript export_map database options =
  try
    let transactions =
      buildImport runtime datascript export_map database options
    in
...
let validateSeededExport runtime datascript export_map schema initial_data
    options =
  let connection = createSeededConnectionWith datascript schema initial_data in
  validateExport runtime datascript export_map
    (Melange_datascript_spec.Api.database datascript connection)
    options
