...
type decoded_datom = {
  entity_id : int;
  attribute : Melange_cljs_runtime_spec.Value_codec.cljs_value;
...
let field_value runtime attribute fields =
  Rrbvec.find_map
    (fun (candidate, value) ->
      if
        Melange_cljs_runtime_spec.Value_codec.value_equals runtime candidate
          attribute
      then Some value
      else None)
    fields
...
let resolve_default_property runtime entities ident =
  let db_ident =
    Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime "db/ident"
  in
  entities
  ...
  | Some callback ->
      let property =
        Melange_cljs_runtime_spec.Value_codec.invoke_callback runtime callback
          ident
      in
      if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime property
      then None
      else Some property
...
let property_is_many runtime datascript property =
  ...
  in
  (not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime cardinality))
...
let assemble runtime datascript datoms entity_fn =
  let decoded =
    ...
    |> Array.map (fun datom ->
        ...
        in
        {
          entity_id =
            datom
            |> Melange_datascript_spec.Api.datom_entity datascript
            |> Melange_cljs_runtime_spec.Value_codec.int_from_value runtime;
          attribute;
          attribute_text =
            Melange_cljs_runtime_spec.Value_codec.keyword_to_string runtime
              attribute;
          value = Melange_datascript_spec.Api.datom_value datascript datom;
        })
  in
  let actions =
    decoded
  ...
  let entity_table =
    {
      by_id = Hashtbl.create (Array.length decoded);
      ordered_ids = Rrbvec.empty;
    }
  in
