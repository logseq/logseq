...
type runtime_conflict_group = {
  key : Melange_cljs_runtime_spec.Value_codec.cljs_value array;
  indices : int Rrbvec.t ref;
  tx : int option;
}

type runtime_replacement_group = {
  entity : Melange_cljs_runtime_spec.Value_codec.cljs_value;
  replacement : Melange_cljs_runtime_spec.Value_codec.cljs_value ref;
}

type runtime_resolution_values = {
  before_lookup : Melange_cljs_runtime_spec.Value_codec.cljs_value option;
  after_tempid : Melange_cljs_runtime_spec.Value_codec.cljs_value option;
}
...
let equal_prefix runtime left right =
  let rec loop index =
    if index = Array.length left then true
      ...
      Melange_cljs_runtime_spec.Value_codec.value_equals runtime left.(index)
        right.(index)
    then loop (index + 1)
    else false
  in
...
let sortDatomsWith runtime datoms =
  ...
  let priority value =
    ...
    if
      Melange_cljs_runtime_spec.Value_codec.value_equals runtime attribute
...
let contains_value runtime values target =
  Rrbvec.exists
    (Melange_cljs_runtime_spec.Value_codec.value_equals runtime target)
    values
...
let retract_entity runtime fields =
  ...
  && Melange_cljs_runtime_spec.Value_codec.value_equals runtime fields.(0)
       (Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime
          "db/retractEntity")
...
let retract_target_id runtime fields =
  if not (retract_entity runtime fields) then None
    ...
    if
      not (Melange_cljs_runtime_spec.Value_codec.value_is_vector runtime target)
    then None
    else
      match
        Melange_cljs_runtime_spec.Value_codec.vector_to_array runtime target
      with
...
let reorderRetractEntityWith runtime tx_data =
  ...
  let add =
    Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime "db/add"
  in
  let uuid_attribute =
    Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime
      "block/uuid"
         ...
         (fun result item_fields ->
           if not (retract_entity runtime item_fields) then result
             ...
             then add_value runtime result entity
               ...
               with
               | [| marker; uuid |]
                 when Melange_cljs_runtime_spec.Value_codec.value_equals runtime
                        marker uuid_attribute ->
  ...
  let retracted =
    ...
    |> Rrbvec.filter_map (fun item ->
        ...
        then None
          ...
          match
            Melange_datascript_spec.Api.entity datascript database fields.(1)
...
let replaceAttrRetractWith runtime datascript database tx_data =
  ...
  let rec group_index entity index =
    if index >= Rrbvec.length !groups then None
      ...
      if
        Melange_cljs_runtime_spec.Value_codec.value_equals runtime group.entity
          entity
      then Some index
  ...
  let as_datom value =
    match
      Melange_datascript_spec.Api.datom_from_value datascript value
      |> Js.Nullable.toOption
    with
    | Some datom -> datom
  ...
  |> Rrbvec.iter (fun value ->
      ...
      if
        Melange_cljs_runtime_spec.Value_codec.value_equals runtime attribute
          block_uuid
        && not (Melange_datascript_spec.Api.datom_added datascript datom)
      then
        let entity_id =
          Melange_datascript_spec.Api.datom_entity datascript datom
        in
        let uuid = Melange_datascript_spec.Api.datom_value datascript datom in
        let uuid_lookup = lookup uuid in
        let existing =
          Melange_datascript_spec.Api.entity datascript database uuid_lookup
          |> Js.Nullable.toOption
        in
        let existing_id =
          match existing with
...
let eid_to_lookup runtime datascript database eid =
  ...
  | Some entity ->
      ...
      if not (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime uuid)
      then
        Some
          (Melange_cljs_runtime_spec.Value_codec.array_to_vector runtime
             [|
...
let ref_value_type runtime datascript database_after database_before attribute =
  ...
  let database_has_ref_type database =
    ...
    | Some entity ->
        ...
        |> Melange_cljs_runtime_spec.Value_codec.value_equals runtime ref_type
  in
...
let normalize_datom_value runtime datascript database_after database_before
  ...
  if
    not
      (Melange_cljs_runtime_spec.Value_codec.value_is_bool runtime added_value)
  then invalid_arg "DB normalization datom added flag must be boolean";
  let added =
    Melange_cljs_runtime_spec.Value_codec.bool_from_value runtime added_value
  in
  let resolve_value =
