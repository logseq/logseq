    ...
    Melange_db.Initial_read.oldest_matching_id_with
      ~datoms:(fun () ->
        Melange_datascript_spec.Api.datoms datascript database (keyword "avet")
          [|
            attribute;
            Melange_cljs_runtime_spec.Value_codec.string_to_value runtime text;
          |])
      ~datom_id:(fun datom ->
    ...
    parse_page_uuid =
      (fun text ->
        if Melange_common.Uuid.is_string text then
          Some
            (Melange_cljs_runtime_spec.Value_codec.uuid_from_string runtime text)
        else None);
  }
...
let inferred_page_reference runtime value =
  if Melange_cljs_runtime_spec.Value_codec.value_is_integer runtime value then
    Domain.Page_id value
  else if Melange_cljs_runtime_spec.Value_codec.value_is_uuid runtime value then
    Domain.Page_uuid value
  else Domain.Page_name (value_name runtime value)

let pageByReferenceWith runtime datascript
    (database : Melange_datascript_spec.Api.database Js.Nullable.t) value =
...
let journalPageWith runtime datascript database value =
  Domain.journal_page_value_with
    ~decode_name:(fun value ->
      if Melange_cljs_runtime_spec.Value_codec.value_is_string runtime value
      then
        Some
          (Melange_cljs_runtime_spec.Value_codec.string_from_value runtime value)
      else None)
...
let hasChildrenByReferenceWith runtime datascript database value =
  let reference =
    if Melange_cljs_runtime_spec.Value_codec.value_is_uuid runtime value then
      Domain.Page_uuid value
    else Domain.Page_id value
  in
  Domain.has_children_with
...
let pageBlocksWith runtime datascript database page_id pattern =
  ...
  Domain.page_blocks_with
    ...
    ~pull_many:(fun pattern ids ->
      Melange_datascript_spec.Api.pull_many datascript database pattern ids)
    ~attribute ~pattern page_id

let pageBlocksByPageWith runtime datascript database
    (page_id : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t)
    pattern =
...
let journalPageByDayInputWith runtime datascript
    ...
    (day : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t) =
...
let keyValueWith runtime datascript database key =
  Domain.key_value_with
    ~entity:(fun key ->
      Melange_datascript_spec.Api.entity datascript database key
      |> Js.Nullable.toOption)
    ~value:(fun entity ->
...
let optionalKeyValueWith runtime datascript
    (database : Melange_datascript_spec.Api.database Js.Nullable.t) key =
  match Js.Nullable.toOption database with
  | None -> Js.Nullable.undefined
...
let pageExistsInputWith runtime datascript database
    (page_name : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t)
    tags =
  match Js.Nullable.toOption page_name with
  | None -> Js.Nullable.undefined
  | Some page_name ->
      let collection =
        Melange_cljs_runtime_spec.Value_codec.value_is_vector runtime tags
        || Melange_cljs_runtime_spec.Value_codec.value_is_set runtime tags
        || Melange_cljs_runtime_spec.Value_codec.value_is_sequential runtime
             tags
        || Melange_cljs_runtime_spec.Value_codec.value_is_map runtime tags
      in
      let tags =
        if collection then tags
        else
          Melange_cljs_runtime_spec.Value_codec.array_to_set runtime [| tags |]
      in
...
let pagesWith runtime datascript database =
  Domain.pages_with
    ~encode_form:(Datalog_runtime.encode runtime)
    ~query:(fun form ->
      Melange_datascript_spec.Api.query datascript form database [||])
    ~collection_to_array:
      (Melange_cljs_runtime_spec.Value_codec.collection_to_array runtime)
    ~row_first:(fun row ->
      match
        Melange_cljs_runtime_spec.Value_codec.collection_to_array runtime row
        |> Array.to_list
      with
      | value :: _ -> value
...
let orphanedPagesWith runtime datascript database pages built_in_pages_names
  ...
  let reference value =
    if Melange_cljs_runtime_spec.Value_codec.value_is_integer runtime value then
      Domain.Page_id value
    else if Melange_cljs_runtime_spec.Value_codec.value_is_uuid runtime value
    then Domain.Page_uuid value
...
let aliasSourcePageWith runtime datascript database alias_id =
  let aliases entity =
    let value = Entity_read.field runtime datascript entity "block/_alias" in
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime value then
      Rrbvec.empty
    else
      value
...
let pageAliasSetWith runtime datascript database page_id =
  let rule =
    Melange_db.Rules.find_body "alias" |> Datalog_runtime.encode runtime
  in
...
let page_order_capabilities runtime datascript database :
  ...
  in
  {
    ...
    order_equal_id = Melange_cljs_runtime_spec.Value_codec.value_equals runtime;
...
let parentsWith runtime datascript database block_id depth =
  ...
  Domain.parents_with
    ~entity:(fun id ->
      Melange_datascript_spec.Api.entity datascript database (lookup id)
      |> Js.Nullable.toOption)
    ~parent:(fun entity ->
...
let allTaggedPagesWith runtime datascript database =
  Domain.all_tagged_pages_with ~encode_form:(Datalog_runtime.encode runtime)
    ~query:(fun form ->
      Melange_datascript_spec.Api.query datascript form database [||])
