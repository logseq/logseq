...
let build_property_with_ident_text runtime (generate_order : value_callback)
  ...
  let title =
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime title_value
    then None
  ...
  let uuid =
    if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime block_uuid
    then Melange_common.Uuid.db_ident_block ~namespace_:(Some namespace_) ~name
    else if
      Melange_cljs_runtime_spec.Value_codec.value_is_uuid runtime block_uuid
    then Melange_cljs_runtime_spec.Value_codec.uuid_to_string runtime block_uuid
    else if
      Melange_cljs_runtime_spec.Value_codec.value_is_string runtime block_uuid
    then
      Melange_cljs_runtime_spec.Value_codec.string_from_value runtime block_uuid
  ...
  Melange_cljs_runtime_spec.Value_codec.map_dissoc runtime property_schema
    (Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime
       "db/cardinality")
...
let db_ident_text_with runtime stable_idents random_index random_bytes db_ident
  ...
  if
    Melange_cljs_runtime_spec.Value_codec.value_is_keyword runtime db_ident
...
let buildClassWith runtime (now_ms : float_callback) block =
  ...
  then invalid_arg "Class DB ident must be a qualified keyword";
  let ident_text =
  ...
  let tags =
    (if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime tags_value
     then [||]
     else
  ...
  let result =
    if
      Domain.add_root_extends ~ident:ident_text
        ~has_extends:
          (not
             (Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime extends))
    then
...
let kvWith runtime key value =
  ...
  then invalid_arg "KV ident must be a qualified keyword";
