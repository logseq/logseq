...
type encoded_options = {
  journals : bool;
  viewForId : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t;
  feature : string Js.Nullable.t;
  groupByPropertyIdent :
    Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t;
  input : string Js.Nullable.t;
  queryEntityIds : Melange_cljs_runtime_spec.Value_codec.cljs_value array;
  query : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t;
  filters : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t;
...
type encoded_result = {
  count : int;
  data : Melange_cljs_runtime_spec.Value_codec.cljs_value;
...
let property_rules runtime =
  ...
  |> Datalog_runtime.encode runtime
...
let selection_capabilities runtime datascript database :
  ...
  let sort_value entity attribute =
    ...
    else if Melange_cljs_runtime_spec.Value_codec.value_is_bool runtime value
    then
      Melange_db.View_order.Bool
        (Melange_cljs_runtime_spec.Value_codec.bool_from_value runtime value)
    else if Melange_cljs_runtime_spec.Value_codec.value_is_number runtime value
    then
      Melange_db.View_order.Number
        (Melange_cljs_runtime_spec.Value_codec.float_from_value runtime value)
    else if Melange_cljs_runtime_spec.Value_codec.value_is_string runtime value
    then
  ...
  in
  {
    ...
    ids_with_bool =
      (fun attribute value ->
        datom_ids attribute
          [|
            Melange_cljs_runtime_spec.Value_codec.bool_to_value runtime value;
          |]);
    ids_with_ref = (fun attribute value -> datom_ids attribute [| value |]);
    with_refs_count =
      (fun entity count ->
        Melange_cljs_runtime_spec.Value_codec.map_assoc runtime entity
          (keyword "block.temp/refs-count")
          (Melange_cljs_runtime_spec.Value_codec.int_to_value runtime count));
    refs_count = Initial_read.blockRefsCountWith runtime datascript database;
    sort_value;
    class_objects =
      (fun id ->
        Class_read.objectsWith runtime datascript database id |> Rrbvec.of_array);
    property_object_ids =
      (fun property_ident ->
        Melange_datascript_spec.Api.query datascript
          (Datalog_runtime.encode runtime Selection.property_objects_query)
          database
        ...
        ({
           ...
           page_counts =
             ...
             |> Option.map (fun counts ->
                 counts
                 |> Array.map
                      (fun (count : Reference_workflow.encoded_page_count) ->
...
let project_group_key runtime datascript value =
  if not (Melange_datascript_spec.Api.entity_is datascript value) then value
  else
    let keyword =
      Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime
    in
    ...
    filter_entities =
      (fun ~filters ~input entities ->
        ...
        View_query_workflow.filterEntitiesWith runtime datascript database
          filters
          (Melange_cljs_runtime_spec.Value_codec.string_to_value runtime input)
          (Rrbvec.to_array entities)
        |> Rrbvec.of_array);
    sort_entities;
    group_entities =
                ...
                |> Rrbvec.map (fun (parent : (_, _) Domain.parent_group) ->
                    let blocks =
                      ...
                      |> Rrbvec.map (fun (block : (_, _) Domain.parent_block) ->
                          Melange_cljs_runtime_spec.Value_codec.entries_to_map
                            runtime
                            [|
...
let getPropertyValuesWith runtime datascript database property_ident
    (view_id : Melange_cljs_runtime_spec.Value_codec.cljs_value Js.Nullable.t)
    query_entity_ids =
  ...
  | None ->
      let property =
        Melange_datascript_spec.Api.entity datascript database property_ident
        |> Js.Nullable.toOption
        |> Option.value
             ~default:(Melange_cljs_runtime_spec.Value_codec.nil_value runtime)
      in
      ...
      let values =
        Melange_datascript_spec.Api.datoms datascript database
          (Melange_cljs_runtime_spec.Value_codec.keyword_from_string runtime
             "avet")
          [| property_ident |]
        |> Array.map (Melange_datascript_spec.Api.datom_value datascript)
      in
