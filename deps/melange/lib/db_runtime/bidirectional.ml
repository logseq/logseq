...
let tag_idents runtime datascript entity =
  Entity_read_runtime.tag_texts runtime datascript entity
...
let getPropertiesWith runtime datascript database target_id =
  ...
  | Some database, Some target_id ->
      ...
      let optional_field value name =
        ...
        if Melange_cljs_runtime_spec.Value_codec.value_is_nil runtime result
        then None
        else Some result
      in
          ...
          Domain.workflow_capabilities =
        {
          query_property_attrs =
            (fun query ->
              Melange_datascript_spec.Api.query datascript
                (Datalog_runtime.encode runtime query)
                database [||]
          ...
          referenced_entity_ids =
            (fun attribute target ->
              ...
              |> Array.map (fun datom ->
                  ...
                  |> Melange_cljs_runtime_spec.Value_codec.int_from_value
                       runtime)
              |> Rrbvec.of_array);
          entity;
          entity_id =
          ...
          created_at =
            (fun value ->
              ...
              |> Option.map
                   (Melange_cljs_runtime_spec.Value_codec.float_from_value
                      runtime));
          classes =
