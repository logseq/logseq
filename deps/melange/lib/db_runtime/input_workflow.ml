...
let lowercase runtime value =
  value
  |> Melange_cljs_runtime_spec.Value_codec.string_lowercase runtime
...
let resolve_keyword runtime datascript database (capabilities : capabilities)
  ...
  | Invalid_relative_format ->
      invalid_arg
        ...
        ^ Melange_cljs_runtime_spec.Value_codec.value_to_string runtime input)
  | Unresolved -> input

let resolveWith runtime datascript database capabilities current_block_uuid
    input =
  if Melange_cljs_runtime_spec.Value_codec.value_is_keyword runtime input then
    resolve_keyword runtime datascript database capabilities current_block_uuid
      input
  else if Melange_cljs_runtime_spec.Value_codec.value_is_string runtime input
  then
    let text =
      Melange_cljs_runtime_spec.Value_codec.string_from_value runtime input
    in
