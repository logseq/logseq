...
let closedValuesWith runtime datascript database property_id =
  ...
  Domain.closed_values_with
    ...
    ~order:(fun value ->
      ...
      if Support.Runtime_codec.value_is_nil runtime order then None
      else Some (Support.Runtime_codec.string_from_value runtime order))
    property_id
...
let closedValueByNameWith runtime datascript database property_id target =
  ...
  Domain.find_closed_value_with
    ...
    ~equals:(Support.Runtime_codec.value_equals runtime)
    property_id target
  |> Js.Nullable.fromOption
