...
let class_value runtime datascript entity =
  Support.entity_tag_texts runtime datascript entity |> fun tags ->
...
let resolve_block runtime datascript database block =
  if Support.Datascript.entity_is datascript block then block
  else
    ...
    let needs_resolution =
      ...
      || Rrbvec.exists (Support.Runtime_codec.value_is_number runtime) tags
    in
    if not needs_resolution then block
    else
      let id = field runtime datascript block "db/id" in
      if Support.Runtime_codec.value_is_number runtime id then
...
let class_conflict runtime datascript database class_entity =
  ...
  | Some database ->
      ...
      if
        Support.Runtime_codec.value_is_nil runtime title
        || Support.Runtime_codec.value_is_nil runtime id
      then false
      else
        Support.Datascript.query datascript
          (Support.encode_datalog_form runtime class_conflict_query)
          database [| title; id |]
...
let inline_tag runtime datascript raw_title tag =
  ...
  | Some raw_title ->
      ...
      if Support.Runtime_codec.value_is_nil runtime uuid then false
      else
...
let uniqueTitleWith runtime datascript database block
  ...
  let tag_titles =
    if (not options.withTags) || class_ then Rrbvec.empty
      ...
      let source_tags =
        if Support.Runtime_codec.value_is_nil runtime source_tags then
          field runtime datascript block_entity "block/tags"
        else source_tags
      in
      let raw_title =
      ...
      |> Rrbvec.map (fun tag ->
          if Support.Runtime_codec.value_is_number runtime tag then
