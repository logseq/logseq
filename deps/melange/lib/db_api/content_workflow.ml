...
let field runtime datascript value name =
  Support.Datascript.entity_get datascript value (keyword runtime name)
...
let values runtime value =
  Support.Runtime_codec.collection_to_array runtime value |> Rrbvec.of_array
...
let required_string runtime label value =
  if not (Support.Runtime_codec.value_is_string runtime value) then
    invalid_arg ("DB content: " ^ label ^ " must be a string");
...
let title_ref_entries runtime datascript refs =
  ...
  |> Rrbvec.filter_map (fun reference ->
      if Support.Runtime_codec.value_is_vector runtime reference then
        match Support.Runtime_codec.vector_to_array runtime reference with
        | [| marker; id |]
        ...
        | _ -> None
      else if Support.Runtime_codec.value_is_map runtime reference then
        match
...
let containsUuidRefWith runtime content =
  Support.Runtime_codec.value_is_string runtime content
  && content
...
let matchedIdsWith runtime content =
  ...
  |> Rrbvec.map (Support.Runtime_codec.uuid_from_string runtime)
  |> Rrbvec.to_array
...
let replaceTitleRefsWith runtime datascript title refs replace_tags =
  if not (Support.Runtime_codec.value_is_string runtime title) then
    invalid_arg "DB content: title must be a string";
...
let replaceUuidInBlockTitleWith runtime datascript entity max_depth
  ...
  if
    (not (Support.Runtime_codec.value_is_string runtime title))
    || not
         (title
         |> Support.Runtime_codec.string_from_value runtime
         |> Domain.contains_uuid_ref)
...
let duplicate_page_name runtime datascript database title =
  ...
  Support.Datascript.datoms datascript database (keyword runtime "avet")
    [|
      ...
      Support.Runtime_codec.string_to_value runtime normalized;
    |]
  |> Array.length
  |> fun count -> count > 1
...
let idRefToTitleRefWith runtime datascript content refs database
  ...
  if not (Support.Runtime_codec.value_is_string runtime content) then
    ...
    |> Domain.replace_id_refs_with_titles
         (Support.Runtime_codec.string_from_value runtime content)
    |> Support.Runtime_codec.string_to_value runtime
...
let updateBlockContentWith runtime datascript database item eid =
  ...
  | Some content ->
      ...
      in
      let refs = Support.Runtime_codec.array_to_vector runtime refs in
      let title =
        idRefToTitleRefWith runtime datascript content refs Js.Nullable.null
          false true
      in
      Support.Runtime_codec.map_assoc runtime item
