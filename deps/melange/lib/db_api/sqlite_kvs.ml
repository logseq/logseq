...
let node_schema_roots database =
  ...
  |> json_string "schema content" |> schema_roots_from_content
...
let node_non_referenced database =
  prepare database non_referenced_addresses_sql |> all
  |> Array.map (fun row ->
         field (json_object row "non-referenced row") "addr"
           "non-referenced row"
...
let node_edges database =
  ...
  |> Array.map (fun row ->
         ...
         let children =
           ...
           |> json_string "edge addresses" |> parse_addresses
         in
         (address, Rrbvec.of_array children))

let wasm_delete database addresses =
  transaction_wasm database
    (fun [@u] tx ->
      Array.iter
        (fun address ->
          ...
          ignore
            (exec tx
               (exec_options_with_bind ~sql:delete_address_sql
                  ~rowMode:"array" ~bind ())))
