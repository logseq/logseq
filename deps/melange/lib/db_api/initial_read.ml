...
let oldestPageByNameInputWith runtime datascript database page_name =
  ...
  | Some database ->
      if Support.Runtime_codec.value_is_string runtime page_name then
  ...
  Domain.oldest_matching_id_with
    ~datoms:(fun () ->
      Support.Datascript.datoms datascript database (keyword "avet")
        [|
          keyword "block/title";
          Support.Runtime_codec.string_to_value runtime page_title;
        |])
    ~datom_id:(fun datom ->
      Support.Datascript.datom_entity datascript datom
      |> Support.Runtime_codec.int_from_value runtime)
    ~eligible:(fun id ->
      let id = Support.Runtime_codec.int_to_value runtime id in
      match
...
let childrenIdsWith runtime datascript database root include_collapsed =
  ...
  in
  let entity_by_id id =
    let lookup = Support.Runtime_codec.int_to_value runtime id in
    match
  ...
  Domain.children_ids_with
    ~root:(fun () ->
      Support.Datascript.entity datascript database root
      |> Js.Nullable.toOption)
    ~entity:entity_by_id
    ~entity_id:(fun entity ->
      field entity "db/id" |> Support.Runtime_codec.int_from_value runtime)
...
let blockRefsWith runtime datascript database root =
  ...
  let collection entity name =
    field entity name |> Support.Runtime_codec.collection_to_array runtime
  in
  ...
  Domain.block_refs_with
    ...
    ~has_ident:(fun entity ident ->
      Support.Datascript.entity_get datascript entity ident
