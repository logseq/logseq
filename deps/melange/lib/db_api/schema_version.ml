...
let decode_parts runtime major minor =
  let major = Support.Runtime_codec.int_from_value runtime major in
  let minor =
    if Support.Runtime_codec.value_is_nil runtime minor then None
    else Some (Support.Runtime_codec.int_from_value runtime minor)
  in
  ...
  let decoded =
    if map_version runtime value then
      ...
      if
        ...
        && Support.Runtime_codec.value_truthy runtime values.(0)
      then
        Some
    ...
    else if Support.Runtime_codec.value_is_integer runtime value then
      Some
        (Domain.make
           (Support.Runtime_codec.int_from_value runtime value)
           None
        |> encode)
...
let stringValueWith runtime value =
  let result =
    if Support.Runtime_codec.value_is_string runtime value then
      ...
      Some
        (Support.Runtime_codec.int_from_value runtime value |> string_of_int)
