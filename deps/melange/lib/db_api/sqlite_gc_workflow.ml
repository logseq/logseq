...
let wasm_capabilities : database Domain.capabilities =
  {
    index_roots = (fun database -> Sqlite_kvs.wasm_schema_roots database |> Rrbvec.of_array);
    non_referenced =
      (fun database -> Sqlite_kvs.wasm_non_referenced database |> Rrbvec.of_array);
...
let node_capabilities : database Domain.capabilities =
  {
    index_roots = (fun database -> Sqlite_kvs.node_schema_roots database |> Rrbvec.of_array);
    non_referenced =
      (fun database -> Sqlite_kvs.node_non_referenced database |> Rrbvec.of_array);
...
let collectWasmDefault (database : database Js.Nullable.t) full_gc =
  Domain.collect_wasm_with wasm_capabilities (Js.Nullable.toOption database)
    ~full_gc
