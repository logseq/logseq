...
let newStorageWith runtime adapter sqlite =
  ...
  let addresses_key =
    Support.Runtime_codec.keyword_from_string runtime "addresses"
  in
  let store address_data _delete_addresses =
    let rows =
      ...
      |> Array.map (fun entry ->
          match Support.Runtime_codec.collection_to_array runtime entry with
          | [| address; data |] ->
              let data, addresses =
                if Support.Runtime_codec.value_is_map runtime data then
                  let addresses =
                    Support.Runtime_codec.map_get runtime data addresses_key
                  in
                  let addresses =
                    if Support.Runtime_codec.value_is_nil runtime addresses
                    then None
                    else Some (stringify_json addresses [@u])
                  in
                  ( Support.Runtime_codec.map_dissoc runtime data
                      addresses_key,
                    addresses )
                else (data, None)
              in
  ...
  let restore address =
    ...
    | Some row -> (
        ...
        | Some addresses
          when Support.Runtime_codec.value_is_map runtime data ->
            Support.Runtime_codec.map_assoc runtime data addresses_key
              addresses
        | Some _ | None -> data)
  in
