...
module Web_crypto = struct
  ...
  external encode : text_encoder -> value -> value = "encode"
  [@@mel.send]

  external digest : subtle -> string -> value -> digest Js.Promise.t
    = "digest"
  [@@mel.send]
  ...
  let buffer_of_value value =
    if String.equal (type_of value) "string" then
      encode (text_encoder ()) value
    else value
...
let checksumValue value =
  Melange_db.Asset.checksum ~buffer_of_value:Web_crypto.buffer_of_value
    ~sha256:Web_crypto.sha256 ~digest_bytes:Web_crypto.digest_bytes value
