...
(** Runtime graph registry decoding and workflow orchestration. *)

module Registry = Melange_common.Graph_registry

type result = { value : Runtime_codec.cljs_value; error : string option }
...
let error_result runtime message =
  { value = Runtime_codec.nil_value runtime; error = Some message }

let value_result value = { value; error = None }
let result_value result = result.value
let result_error result = result.error

let normalize_value runtime value =
...
let upsert_value runtime registry value =
  let entry = value_entry runtime value in
  match Registry.normalization_error entry with
  | Some message -> error_result runtime message
  | None ->
      let registry =
        if Runtime_codec.value_is_nil runtime registry then Rrbvec.empty
          ...
          Runtime_codec.collection_to_array runtime registry |> Rrbvec.of_array
      in
...
let resolve_target_value runtime registry graph_id graph_identifier =
  let registry =
    if Runtime_codec.value_is_nil runtime registry then Rrbvec.empty
