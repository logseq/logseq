(ns ^:node-only logseq.melange.bridge.db.test-helper
...
(defn readable-properties
  ...
  [ent]
  (->> (melange-property/properties ent)
       (mapv (fn [[k v]]
               [k
                (cond
                  ...
                  (de/entity? v)
                  (or (:db/ident v) (melange-property/property-value-content v))
                  :else
                  v)]))
       (into {})))
...
(defn create-conn-with-import-map
  ...
  (let [conn (create-conn)
        ...
        (sqlite-export/build-import (dissoc export-map :logseq.db.sqlite.export/graph-files) @conn {})
        _ (d/transact! conn (concat init-tx block-props-tx misc-tx))
        ;; Handle graph-files separately b/c build-import can't upsert them
        _ (when (:logseq.db.sqlite.export/graph-files export-map)
            (d/transact! conn (:logseq.db.sqlite.export/graph-files export-map)))]
    conn))
