...
(defn config-from-env []
  (let [env (.-env js/process)]
    {:port (when-let [v (env-value env "DB_SYNC_PORT")] (parse-int v 8080))
     ...
     :cloudflare-sandbox-bootstrap-command (env-value env "CLOUDFLARE_SANDBOX_BOOTSTRAP_COMMAND")
     :cloudflare-sandbox-agent-port (env-value env "CLOUDFLARE_SANDBOX_AGENT_PORT")
     :cloudflare-sandbox-hostname (env-value env "CLOUDFLARE_SANDBOX_HOSTNAME")
     :cloudflare-sandbox-port-token (env-value env "CLOUDFLARE_SANDBOX_PORT_TOKEN")
     :cloudflare-sandbox-memory-mb (env-value env "CLOUDFLARE_SANDBOX_MEMORY_MB")
     :cloudflare-sandbox-sleep-after (env-value env "CLOUDFLARE_SANDBOX_SLEEP_AFTER")
     :cloudflare-sandbox-health-retries (env-value env "CLOUDFLARE_SANDBOX_HEALTH_RETRIES")
     :cloudflare-sandbox-health-interval-ms (env-value env "CLOUDFLARE_SANDBOX_HEALTH_INTERVAL_MS")
