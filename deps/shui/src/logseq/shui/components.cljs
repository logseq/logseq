...
(def TabsIndicatorPart (primitive-part TabsPrimitive "Indicator"))
...
(defn- tabs-list-class
  [props]
  (case (prop-name (prop props "variant"))
    "line" "ui__tabs-list relative inline-flex h-auto items-center justify-start bg-transparent p-0 text-muted-foreground"
    "ui__tabs-list inline-flex h-10 items-center justify-center rounded-md bg-muted p-1 text-muted-foreground"))

(defn- tabs-trigger-class
  [props]
  (case (prop-name (prop props "variant"))
    "line" "ui__tabs-trigger inline-flex items-center justify-center whitespace-nowrap px-0 pb-3 pt-0 text-base font-medium text-muted-foreground transition-colors focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 data-[active]:text-foreground aria-selected:text-foreground"
    "ui__tabs-trigger inline-flex items-center justify-center whitespace-nowrap rounded-sm px-3 py-1.5 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[active]:bg-background data-[active]:text-foreground data-[active]:shadow-sm aria-selected:bg-background aria-selected:text-foreground aria-selected:shadow-sm"))

(def TabsIndicator
  (forward-part TabsIndicatorPart "ui__tabs-indicator absolute bottom-0 left-[var(--active-tab-left)] h-[3px] w-[var(--active-tab-width)] bg-foreground transition-all duration-200 ease-out"))

(def TabsList
  (react/forwardRef
   (fn [^js props ref]
     (let [line? (= "line" (prop-name (prop props "variant")))
           props' (with-class-props props (tabs-list-class props) nil)]
       (when ref (set-prop! props' "ref" ref))
       (clean-props! props' "variant")
       (if line?
         (react/createElement TabsListPart props'
                              (prop props' "children")
                              (react/createElement TabsIndicator nil))
         (react/createElement TabsListPart props'))))))

(def TabsTrigger
  (react/forwardRef
   (fn [^js props ref]
     (let [props' (with-class-props props (tabs-trigger-class props) nil)]
       (when ref (set-prop! props' "ref" ref))
       (clean-props! props' "variant")
       (react/createElement TabsTabPart props')))))
