...
(defn- check-page-password
  ...
  (p/let [stored-hash (fetch-page-password-hash graph-uuid page-uuid env)]
    (if (string/blank? stored-hash)
      {:allowed? true :provided? false}
      (let [provided (request-password request)]
        (if (string? provided)
          (p/let [valid? (publish-common/verify-password provided stored-hash)]
      ...
      (p/let [{:keys [claims]} (auth-claims request env)]
        (if (nil? claims)
          ...
          (let [^js do-ns (aget env "PUBLISH_META_DO")
                index-id (.idFromName do-ns "index")
                index-stub (.get do-ns index-id)]
            (p/let [resp (.fetch index-stub (str "https://publish/short/" short-id)
                                 #js {:method "GET"})]
              (if-not (.-ok resp)
                (publish-common/not-found)
                (p/let [data (.json resp)
  ...
  (let [url (js/URL. (.-url request))
    ...
    (cond
      ...
      (and (or (string/starts-with? path "/p/")
           ...
           (= method "DELETE"))
      (handle-delete-page-by-short-id request env)
