...
- Commands and results:
  ...
  - `rtk pnpm --dir the removed DB dependency project test-v logseq.db-test/batch-transact-with-temp-conn-preserves-cardinality-one-schema-test logseq.db-test/batch-transact-with-temp-conn-preserves-retracts-test`: completed the the removed DB dependency project suite, 114 tests, 418 assertions, 0 failures.
  - `rtk pnpm --dir the removed DB dependency project test-v logseq.db-test/batch-transact-with-temp-conn-before-commit-can-abort-live-commit-test logseq.db-test/batch-transact-with-temp-conn-preserves-cardinality-one-schema-test logseq.db-test/batch-transact-with-temp-conn-preserves-retracts-test logseq.db-test/transact-sync-retries-when-live-conn-changes-before-commit-test`: the targeted assertions printed as passing, but the full the removed DB dependency project runner later stopped producing output in `gc-kvs-table-test` and was interrupted, so this invocation is not counted as a completed pass.
...
- Status: completed commands passed; the latest the removed DB dependency project retry was interrupted after target assertions printed as passing and is not counted as a completed pass.
...
- Test evidence:
  ...
  - `rtk pnpm --dir the removed DB dependency project test-v logseq.db-test/batch-transact-with-temp-conn-before-commit-can-abort-live-commit-test logseq.db-test/batch-transact-with-temp-conn-preserves-cardinality-one-schema-test logseq.db-test/batch-transact-with-temp-conn-preserves-retracts-test`: the removed DB dependency project suite completed, 115 tests, 420 assertions, 0 failures.
  - `rtk node --test scripts/cli-concurrent-edit-stress.test.mjs scripts/cli-sync-stress-workflow.test.mjs`: 30 tests, 0 failures.
...
- Regression tests:
  ...
  - Rebase adjacency still passes:
    ...
    `frontend.worker.db-sync-test/apply-remote-txs-with-local-changes-rejects-invalid-final-rebase-test`.
- Test evidence:
  - `rtk pnpm --dir the removed DB dependency project test-v logseq.db-test/batch-transact-with-temp-conn-preserves-cardinality-one-schema-test` completed the the removed DB dependency project suite: 114 tests, 418 assertions, 0 failures.
