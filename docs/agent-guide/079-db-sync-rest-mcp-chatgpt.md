...
Task, page, tag-object, tag, property, and search-result collections also accept `created-after` and `updated-after` as Unix epoch timestamps in milliseconds. Comparisons are strict; when both are supplied, an entity must satisfy both. Time filtering happens before pagination, and entities without the requested timestamp are excluded.
...
Assets remain blocks tagged with `:logseq.class/Asset`. `GET /assets` lists asset blocks with cursor pagination and time filters. `POST /assets` accepts a raw binary request body plus declared file name, exact byte size, and client-computed SHA-256 checksum. Callers such as ChatGPT Code Mode that can only transfer a string use `encoding=base64`; the trusted MCP host validates the base64 and recalculates its decoded byte size instead of trusting the agent-provided value. The Durable Object then decodes that body as a stream before R2, while `checksum` continues to describe the decoded file. The decoded body is wrapped in a fixed-length stream and passed directly to R2; the Durable Object never calls `arrayBuffer()` or realizes the full payload in memory. Values over 100MB are rejected before R2. After R2 accepts the stream, the handler creates the asset block through `deps/outliner`; if block creation fails, it deletes the new R2 object. Duplicate checksums return `409`. `GET /assets/:asset-block-id` returns a five-minute signed Worker URL without an OAuth bearer token. Asset deletion uses the same `DELETE /blocks/:block-id` operation as every other block.
